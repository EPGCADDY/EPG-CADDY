...
assert.match(html,/requestedByLink=startupParams\.get\("recover_stableford"\)==="20260904"/);
assert.match(html,/const recoveryPreviewHost=location\.hostname\.startsWith\("epg-caddy-git-v376-stableford-continu-"\)\|\|location\.hostname\.startsWith\("epg-caddy-git-v381-unified-"\)/);
assert.match(html,/startupParams\.get\("source"\)==="pwa"&&recoveryPreviewHost&&!isRecoverableStoredRound\(loadRound\(\)\)/);
...
assert.match(html,/if\(recoveredStablefordSeptember4&&!round\.officiallyClosedAt\)void officiallyCloseRound\(\)/);
