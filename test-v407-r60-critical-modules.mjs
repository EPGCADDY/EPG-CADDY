...
const current="V407-R60-UNIVERSAL-RECOVERY-20260912";
...
assert.equal(elements.appVersionId.textContent,"V407 · R60 · 12/09/2026");
await window.GSCUpdatePinned.check();
assert.equal(classes.has("available"),false,"R52 no puede activar un downgrade desde R60");
assert.equal(elements.mandatoryUpdateButton.disabled,true);

remote="V407-R61-BROWSER-PROBE-20260912";
await window.GSCUpdatePinned.check();
assert.equal(classes.has("available"),true,"R61 debe activar ACTUALIZAR desde R60");
...
console.log("PASS R60 CRÍTICO · downgrade bloqueado, upgrade instalado, caché retirada, datos preservados y V/R/fecha visibles");
