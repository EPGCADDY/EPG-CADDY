...
const expected='<a class="live-support-link" href="/manual.pdf?__gscg_build_check=1" aria-label="Abrir el Manual de Golf Score Card GT en esta pantalla">Support</a>';
...
assert.match(worker,/\["\/manual\.pdf","\/manual\.html","\/manual-scg"\]\.includes\(url\.pathname\)/,"El service worker debe dejar pasar el PDF y el Manual sin devolver la aplicación");
assert.match(worker,/url\.searchParams\.has\("__gscg_build_check"\)/,"El service worker anterior debe dejar pasar Support por red");
assert.ok(!html.includes('class="live-support-link" href="/manual.pdf" aria-label='),"Support no puede quedar vulnerable al shell de un PWA anterior");
