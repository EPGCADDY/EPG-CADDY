...
assert.match(control,/viewer\.id="liveViewerSection"/,"el visor público queda separado de los controles de la ronda");
assert.match(control,/const hasRound=!!currentSnapshot\(\)/,"LIVE detecta cualquier ronda activa");
assert.match(control,/viewer\?\.classList\.toggle\("hidden",hasRound\)/,"una ronda activa no abre el menú público genérico");
assert.match(control,/organizer\?\.classList\.toggle\("hidden",!hasRound\)/,"toda ronda activa abre directamente sus controles LIVE");
for(const mode of ["general","match_play","four_ball","stableford","universales"])assert.match(control,new RegExp(`\\[.*${mode}`),`LIVE conserva ${mode}`);
