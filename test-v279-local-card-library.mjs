...
assert.equal(entries.length,2,"La biblioteca sólo admite snapshots oficiales");
...
assert.doesNotMatch(html,/function openCardLibrary[\s\S]{0,900}round=/,"Abrir la biblioteca no reemplaza la ronda actual");

console.log("PASS V279 · biblioteca privada local por ronda, campo, torneo, fecha y jugador sin alterar la ronda actual");
