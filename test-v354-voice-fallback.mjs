...
assert.match(processSource,/browser_fallback_general_in_place/);
...
const makeBrowserProcessor=(parseResult,submitResult=true)=>new Function(
  ...
  async()=>{runtimeOrder.push("submit");return submitResult},()=>({textContent:""}),{configured:true},
);
...
assert.equal(runtimeEvents.some(item=>item.event==="browser_fallback_general_in_place"),true);
...
console.log("PASS V354/V367 VOZ: 3 hoyos, plural y consulta general audible sin abandonar la pantalla");
