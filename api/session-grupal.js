...
export default async function handler(req, res) {
  ...
  try {
    ...
    const transcriptionPrompt = context === "setup"
      ...
      : `Golf Guatemala. Dictado de scores. Jugadores actuales: ${players || "los registrados en la tarjeta"}. El cursor ya indica automáticamente el hoyo activo: transcribe literalmente nombre y score o Gross; el hoyo es opcional y solo reposiciona el cursor cuando se pronuncia. Después de Falta NOMBRE, una respuesta con solo score o solo omisión pertenece exclusivamente a ese jugador. Acepta número Gross directo, vocabulario golfístico y omisiones explícitas: X, equis, cero, sin score, sin dato, sin resultado, no informó, no reportó, no dijo, no cantó, no dio score, no se sabe, ponle cero y no le anotes. Vocabulario golfístico: albatros, águila, aguiler, eagle, birdie, pájaro, verdura, par, even par, parinelo, paraso, parcuato, bogey, doble bogey, doblete, triple bogey, triplete, doble par, par español, uno bajo par, uno sobre par, dos sobre par y tres sobre par. Si un primer nombre es único en el grupo puede dictarse solo ese nombre; si hay dos jugadores con el mismo primer nombre, puede dictarse únicamente el apellido. Regla mandatoria: Jessie se escribe Jessie.`;
