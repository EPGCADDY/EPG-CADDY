...
export function validateMutation(input) {
  ...
  const mutation = {
    clientMutationId: String(input.clientMutationId || ""), installationId: String(input.installationId || ""),
    entityType: String(input.entityType || ""), entityId: String(input.entityId || ""),
    payloadHash: String(input.payloadHash || "").toLowerCase(), payload: input.payload
  };
  for (const key of ["clientMutationId", "installationId", "entityType", "entityId"]) {
    if (!ID_PATTERN.test(mutation[key])) throw Object.assign(new Error(`INVALID_${key.toUpperCase()}`), { code: "INVALID_MUTATION" });
  }
