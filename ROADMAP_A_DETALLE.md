...
- Causa del `reference_error`: `voice-assistant.js` registraba `DOMContentLoaded` también durante importación Node. R65 protege la llamada con `typeof window!=="undefined"`; en iPhone mantiene WebRTC persistente y fallback local. Evidencia: `test-v407-r63-iphone-persistent-voice.mjs`. MAESTRO intacto.
...
- Se elimina la misma identidad R32 fija en ocho gates visuales de modalidades, propietario, tarjeta, torneo y diseño; cada uno mantiene intacta su validación funcional y acepta la revisión V407 activa.
