...
## Registro técnico V328 · Reglas oficiales dentro de AI UNIVERSAL ∞

El centro reglamentario reutiliza panel, conversación temporal, micrófono bilateral, síntesis, fuentes y contexto de la aplicación. `api/golf-rules.js` obliga a investigar en USGA/The R&A, filtra de nuevo las fuentes recibidas y falla si no existe autoridad oficial. La tarjeta entrega únicamente campo y modalidad; no expone coordenadas, nombres ni scores. El modo REGLAS evita deliberadamente `routeAiUniversalAppText`, por lo que una consulta no puede convertirse en escritura. El árbol local y el remoto coinciden exactamente en `41bafb27bf2afe381789b7b2315f32860f764dd5`; el primer deployment V328 quedó `READY` después de aprobar 86 paquetes.
...
| `vercel.json` | `AUDIT 86 + LIVE RULE GATE` | Obliga regresión completa y consulta oficial real antes de entregar cada Preview V328. |
...
| `service-worker.js` | `gscg-mobile-v328-official-golf-rules` | Fuerza sustitución de la copia instalada anterior. |
| `scripts/update-inventory-v328.py` | `3 INVENTORY COVERS / PREVIEW READY / IDEMPOTENT` | Actualiza la portada V328-R1 de los tres inventarios sin duplicarla al repetir el proceso. |
| `audit-project.mjs` | `test-v328-official-golf-rules.mjs` | Agrega el paquete reglamentario a la regresión maestra. |
| `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `GOLF_SCORE_CARD_GT_GRUPAL_MANUAL_MAESTRO.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md` | `PEND-REG-001 V328-R1 PREVIEW READY` | Distinguen Preview validado de voz física, offline y eventual licencia comercial todavía pendientes. |
