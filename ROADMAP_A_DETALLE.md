...
## 2026-09-18 · Voice direct connection deadline (audible validation pending)
Scope: api/_lib/universal-pcm.js and its focused regression tests. Direct speech headers now have a 1500 ms deadline; timeout, network error, or HTTP rejection can fall back once to the same tts-1 / Onyx / speed 0.9. Caller cancellation never falls back; failure after audio starts never restarts speech. Gateway logs now distinguish direct_not_configured, direct_headers_timeout, direct_network_error, and direct_http_STATUS without exposing credentials or question text.
Evidence: node test-universal-pcm.mjs PASS with simulated providers, including blocked connection, network failure, HTTP 401, caller cancellation, partial-stream failure, two turns and compressed playback cancellation. These tests do not certify actual latency, accent, or audible identity.
Owner reports 17 seconds by stopwatch for a simple coffee question. The prior 6855 ms browser scheduling event was not confirmed as that question and is not audible onset. Two-second acceptance remains FAILED / unverified. Gateway TTS officially does not support streaming output: https://vercel.com/docs/ai-gateway/modalities/text-to-speech#limitations . No provider credentials or configuration were changed. Preview only; Production and official LAB unchanged. Rollback: 5a6858f7fef44ae3d584a7e43d65e40167849d8b. Limitation: this patch cannot improve a turn that already goes straight to Gateway; identifying the deployed route requires its new route event. A 1500 ms direct-header deadline can also discard a direct request that would have succeeded later, so it is not a guaranteed end-to-end latency reduction.
