...
### V371-R10 · sesión de audio renovada por cada gesto

Los logs físicos R9 de las 20:01–20:02 Guatemala muestran una primera respuesta completa, seguida por tres ventanas de ocho segundos con `browser_fallback_no_result_timeout`, dos reinicios y cierre `aborted`. La causa es conservar el objeto `Audio` entre respuestas: evita el bloqueo de reproducción, pero la sesión de salida continúa interfiriendo con `SpeechRecognition`. `releaseAiUniversalPlaybackForListening` vuelve a destruir el reproductor; `fireMicActivation` lo libera primero y `primeAiUniversalSpeechFromGesture` crea uno nuevo dentro de cada toque para autorizar la respuesta posterior. `directTrafficRouteFromQuery` agrega el conector `para`, y la respuesta hablada directa se acorta para reducir el tiempo de TTS. Archivos funcionales: `index-grupal.html`, `api/universal-ai.js` y caché `service-worker.js`; pruebas dirigidas V355/V369/V370 y controles de caché V357/V361. Registro y Scores no cambian. Producción permanece intacta.

### V371-R11 · país del GPS aplicado al destino de tráfico

La prueba física R10 resolvió la frase completa, pero Google interpretó `Oakland Mall zona 10` como Oakland, Estados Unidos. La causa fue verificable: `computeTrafficRoute` enviaba las coordenadas únicamente como origen y dejaba el destino ambiguo sin región. R11 consulta a Google Geocoding el país correspondiente al GPS y envía su código CLDR/ISO de dos letras como `regionCode` a Google Routes; el mismo mecanismo funciona en cualquier país y no contiene una excepción para Guatemala. `test-v324-real-traffic.mjs` exige que un origen GPS guatemalteco produzca `regionCode: GT`; `test-v357-ios-voice-transport-recovery.mjs` y `test-v361-synchronized-voice.mjs` fijan el nuevo caché R11. Registro, Scores, cálculos deportivos, persistencia e interfaz permanecen sin cambios. Producción permanece intacta.

### V372 · Gateway Realtime para continuidad y primera palabra rápida

Con autorización expresa del propietario se reemplaza exclusivamente el transporte compartido, sin modificar parsers, escritores, cálculos ni persistencia. `api/realtime-gateway-token.js` entrega un secreto efímero de cinco minutos para `openai/gpt-realtime-2.1`; `gateway-realtime.js` captura PCM, usa cierre local tras 1.2 segundos de silencio, recibe la transcripción y la entrega a `processBrowserVoiceTranscript`, exactamente la misma función ya protegida. Las respuestas se reproducen como fragmentos PCM Cedar mientras se generan; `/api/voice-speech` y Safari quedan únicamente como recuperación. `test-v372-gateway-realtime-streaming.mjs` fija precedencia, idioma, silencio, transcripción, audio progresivo y aislamiento; `package.json`, `package-lock.json`, `audit-project.mjs`, `service-worker.js` e `index-grupal.html` integran el control. Producción permanece intacta.

### V372-R1 · recuperación del empaquetado remoto sin cambio funcional

El despliegue `dpl_2fvEhm94ZvtTU95seiJM98UtjorZ` fue rechazado internamente porque la transferencia inicial dejó incompleto `index-grupal.html`; el control `ACTIVE_ROUND_KEY` lo detectó. El archivo se volvió a transferir completo y su blob remoto coincide con el local (`37e5c24f0e8bfd26d4543cd4976b525365f2e215`). El segundo constructor exigió que el commit reparador actualizara simultáneamente ambos ROADMAPS; R1 registra ese punto de escape, regenera el inventario y repite las tres auditorías antes de un nuevo despliegue. Cero modificación funcional adicional; Producción intacta.
