...
### V374 · transporte PCM por la ruta Realtime de transcripción

La misma versión V373 midió 14.101 s, 11.966 s y 11.980 s en tres transcripciones reales. La causa es arquitectónica: `MediaRecorder` cerraba el archivo antes de iniciar la petición por lotes. V374 modifica exclusivamente el transporte: `server-voice-capture.js` convierte y envía PCM mono de 24 kHz durante la captura; `api/voice-transcribe.js` entrega un token efímero limitado a `openai/gpt-realtime-whisper` con `routeKind=transcription`; al segundo de silencio envía `transcription-stream.audio-done` y entrega el resultado a la misma `processBrowserVoiceTranscript()`. `service-worker.js` invalida únicamente la copia anterior; `test-v373-server-voice-transcription.mjs` bloquea retorno a MediaRecorder y valida protocolo, modelo, idioma y token protegido; `test-v357-ios-voice-transport-recovery.mjs` y `test-v361-synchronized-voice.mjs` fijan la nueva firma de caché. Registro, Scores, cálculos, persistencia, clima, tráfico, interfaz y Producción no cambian.

V374-R1: el deployment `dpl_AsKycPsxba4quzKAX6SWHeKNEfNw` quedó READY, pero el token `openai/gpt-realtime-whisper` devolvió 403. `api/voice-transcribe.js` prueba en orden los tres modelos con streaming que publica el catálogo vivo del Gateway: OpenAI, `google/gemini-3.5-transcribe-live` y `spacexai/grok-stt`; sólo entrega el primer token autorizado. `test-v373-server-voice-transcription.mjs` reproduce el 403 y exige el salto a Google sin exponer credenciales. Ningún otro archivo funcional cambia.

V374-R2: la causa del rechazo quedó aislada en la autenticación OIDC del token experimental, no en el saldo ni en la dirección de facturación. Una clave AI Gateway secreta queda asignada exclusivamente a la rama Preview `lab/recovery-production-approved-v363`. `api/voice-transcribe.js` reutiliza una sola función de emisión y ofrece una sonda `GET ?health=stream-token` únicamente cuando `VERCEL_ENV` no es `production`; devuelve sólo estado, modelo y milisegundos. `test-v373-server-voice-transcription.mjs` exige que la sonda no exponga `token` y que Producción responda 405. Registro, Scores, interfaz, datos y Producción no cambian.
