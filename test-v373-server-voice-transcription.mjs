...
assert.doesNotMatch(capture,/action:"stream-token"/);
assert.doesNotMatch(capture,/new WebSocket/);
assert.match(capture,/TARGET_RATE=24000/);
assert.match(capture,/if\(requestUniversalOnly&&typeof MediaRecorder==="function"\)[\s\S]*?new MediaRecorder/);
