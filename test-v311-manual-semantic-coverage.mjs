...
assert.equal(pages.length,56,"Deben existir exactamente las páginas funcionales 17–72");
assert.deepEqual(pages.map(page=>Number(page.number)),Array.from({length:56},(_,index)=>index+17));
...
const required=[
  ...
  "MAPA DE CAPACIDADES DE VOZ","QUÉ RESPONDERÁ LA APLICACIÓN"
];
...
console.log("PASS V311 · 72 páginas funcionales con mapa de voz, respuestas y recuperación de atascos");
