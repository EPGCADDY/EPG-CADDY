...
assert.match(html,/input_audio_buffer\.speech_started"&&voiceContext==="round"&&listening\)\{\s*roundSpeechActive=true;\s*cancelOperationalMissingTimer\(\)/);
assert.match(html,/conversation\.item\.input_audio_transcription\.delta"&&voiceContext==="round"&&!stopMonitorActive\)\{\s*roundSpeechActive=true;\s*cancelOperationalMissingTimer\(\)/);
