...
for(const required of ['candidate-index-grupal.html','index-grupal.html','middleware.js'])if(!payloadFiles.includes(required))throw new Error(`REQUIRED_RUNTIME_MISSING_${required}`);
const vercel=JSON.parse(text('vercel.json'));
if(!String(vercel.buildCommand||'').trim().startsWith('node scripts/update-chain-gate.mjs &&'))throw new Error('VERCEL_CHAIN_GATE_NOT_FIRST');
...
const lock={schema:'gscg-update-chain-payload/v4',status:'PUBLICATION_CHAIN_LOCKED',release,fileCount:payloadFiles.length,fingerprint,candidateSha256:files['candidate-index-grupal.html'],generatedFromCommit:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),scope:'runtime-auto-v3-vercel-semantic',vercelContract:{gateFirst:true},files};
fs.mkdirSync('Intocables',{recursive:true});
fs.writeFileSync(manifestPath,JSON.stringify(lock,null,2)+'\n');
console.log(`UPDATE_CHAIN_LOCK WRITTEN release=${release} files=${payloadFiles.length} fingerprint=${fingerprint.slice(0,12)} vercel=semantic`);
