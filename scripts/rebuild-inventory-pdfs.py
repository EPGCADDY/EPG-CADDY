...
def markdown_pdf(source, target, title):
    ...
    story = [Spacer(1, 45 * mm), Paragraph(html.escape(title), cover), Paragraph("Fuente: candidato V366-PRINCIPAL-ENTRY-RECOVERY integrado con voz OIDC + INTOCABLES - Produccion intacta", ParagraphStyle("CoverSub", parent=base, alignment=TA_CENTER, fontSize=10)), PageBreak()]
...
def image_inventory_pdf(target):
    ...
    document.drawCentredString(A4[0] / 2, A4[1] - 70 * mm, "Base visual V292 + candidato V366 integrado con voz OIDC + INTOCABLES")
    document.showPage()
...
def write_lock(paths):
    ...
    payload = {
        "version": "V366-PRINCIPAL-ENTRY-RECOVERY-OIDC-INTOCABLES",
...
def main():
    ...
    print(f"INVENTORY_LOCK PASS version=V366-PRINCIPAL-ENTRY-RECOVERY-OIDC-INTOCABLES sources={json.loads(LOCK.read_text(encoding='utf-8'))['sourceFileCount']}")
