...
def markdown_pdf(source, target, title):
    ...
    story = [Spacer(1, 45 * mm), Paragraph(html.escape(title), cover), Paragraph("Fuente: candidato V360-INTEGRATED-PROGRESSIVE-PARSER - Produccion intacta", ParagraphStyle("CoverSub", parent=base, alignment=TA_CENTER, fontSize=10)), PageBreak()]
...
def image_inventory_pdf(target):
    ...
    document.drawCentredString(A4[0] / 2, A4[1] - 70 * mm, "Base visual V292 + registro candidato V360-INTEGRATED-PROGRESSIVE-PARSER")
    document.showPage()
...
def write_lock(paths):
    ...
    payload = {
        "version": "V360-INTEGRATED-PROGRESSIVE-PARSER",
...
def main():
    ...
    print(f"INVENTORY_LOCK PASS version=V360-INTEGRATED-PROGRESSIVE-PARSER sources={json.loads(LOCK.read_text(encoding='utf-8'))['sourceFileCount']}")
