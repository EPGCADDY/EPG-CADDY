...
def markdown_pdf(source, target, title):
    ...
    story = [Spacer(1, 45 * mm), Paragraph(html.escape(title), cover), Paragraph("Fuente: candidato V354-VOICE-FALLBACK-MULTIHOLE-GENERAL - Produccion intacta", ParagraphStyle("CoverSub", parent=base, alignment=TA_CENTER, fontSize=10)), PageBreak()]
...
def image_inventory_pdf(target):
    ...
    document.drawCentredString(A4[0] / 2, A4[1] - 70 * mm, "Base visual V292 + registro candidato V354-VOICE-FALLBACK")
    document.showPage()
...
def write_lock(paths):
    ...
    payload = {
        "version": "V354-VOICE-FALLBACK-MULTIHOLE-GENERAL",
...
def main():
    ...
    print(f"INVENTORY_LOCK PASS version=V354-VOICE-FALLBACK-MULTIHOLE-GENERAL sources={json.loads(LOCK.read_text(encoding='utf-8'))['sourceFileCount']}")
