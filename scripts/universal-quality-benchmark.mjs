...
const results=await Promise.all(questions.map(async(query,index)=>{
...
console.log('UNIVERSAL_BENCHMARK_SUMMARY '+JSON.stringify({answered:results.filter(x=>x.status===200&&x.answer).length,total:4,quality:'REQUIRES_HUMAN_COMPARISON_NOT_AUTOMATIC_PASS'}));
