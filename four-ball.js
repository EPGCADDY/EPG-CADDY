(function(root,factory){
  ...
  function teamIndexForPlayer(playerIndex){const index=Number(playerIndex);return index===0||index===1?0:index===2||index===3?1:null}
  ...
  function holeResult(players,hole){
    ...
    if(count===1)return{hole:number,recorded:true,scores,teamBest,teamStatuses:["solo"],playerStatuses:Array(2).fill("tied"),winnerTeamIndex:null};
  ...
  function status(players,{maxHoles=MAX_HOLES,names=DEFAULT_TEAM_NAMES}={}){
    ...
    if(!count)return{valid:false,teamCount:0,played:0,remaining:limit,closed:false,label:"FOUR BALL REQUIERE 2 O 4 JUGADORES",resultLabel:"FOUR BALL REQUIERE 2 O 4 JUGADORES",teamNames:labels,holes:[]};
    const holes=[];let winsA=0,winsB=0,halves=0,totalNet=0;
    for(let hole=1;hole<=limit;hole++){
      ...
      else if(result.winnerTeamIndex===0)winsA++;else if(result.winnerTeamIndex===1)winsB++;else halves++;
    }
    ...
    return{valid:true,teamCount:2,played,remaining,wins:[winsA,winsB],halves,margin,lead,leaderTeamIndex,leader,closed,decidedAt:closed?played:null,label,resultLabel,teamNames:labels,holes};
  }
  ...
  function teamStanding(players,teamIndex,holes){
    ...
    let won=0,tied=0,lost=0,played=0,netTotal=0;
    for(const hole of selected){
      ...
      else{const state=result.teamStatuses[index];if(state==="won")won++;else if(state==="lost")lost++;else tied++}
    }
