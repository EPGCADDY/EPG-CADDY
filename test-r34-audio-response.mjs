...
const blocked=fixture({rejectPlay:true});blocked.env.window.GSCVoiceTurns={enabled:true};assert.equal(await blocked.env.speakAiUniversalText('Se puede leer aunque Safari bloquee el audio'),false);assert.match(blocked.nodes.get('universalSpokenAnswerText').textContent,/Se puede leer/);assert.ok(blocked.events.includes('browser_fallback_speech_failed'));assert.equal(blocked.nodes.get('universalSpokenAnswerText').hidden,true);assert.equal(blocked.nodes.get('universalSpokenAnswer').hidden,true);console.log('PASS rechazo PTT devuelve false sin mostrar texto ni recuadro, conforme a orden del propietario');
