...
const blocked=fixture({rejectPlay:true});blocked.env.window.GSCVoiceTurns={enabled:true};assert.equal(await blocked.env.speakAiUniversalText('Se puede leer aunque Safari bloquee el audio'),false);assert.match(blocked.nodes.get('universalSpokenAnswerText').textContent,/Se puede leer/);assert.ok(blocked.events.includes('browser_fallback_speech_failed'));assert.equal(blocked.nodes.get('universalSpokenAnswerText').hidden,false);console.log('PASS rechazo de reproducción PTT conserva texto visible y controles, devuelve false');
