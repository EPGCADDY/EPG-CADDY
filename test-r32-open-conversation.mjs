...
const apiEnv=vm.createContext({
  $:element,aiUniversalTextBusy:false,aiUniversalRulesMode:false,aiUniversalMuted:false,
  ...
  fetch:async()=>({ok:true,status:200,headers:{get:()=> 'application/json'},json:async()=>({ok:true,answer:'Respuesta de prueba',sources:[]})}),
  speakAiUniversalText:async()=>delivered,
  reportVoiceHealth:event=>events.push(event),phase:'idle'
});
vm.runInContext(section('async function readUniversalVoiceResponse(', '\nasync function continueConversationAfterTool(')+'\n'+
