...
## V354-R2 · RC-024 tráfico local por voz

`api/_lib/traffic.js` concentra la geocodificación inversa privada y el contexto dinámico ciudad/estado/país; `api/traffic.js` lo aplica a la herramienta Realtime, `api/universal-ai.js` al respaldo determinista e `index-grupal.html` obtiene el GPS autorizado sin obligar a dictar el país. Un origen escrito no se sustituye por el GPS; si la orden sólo incluye destino, el GPS sí funciona como origen. `api/weather.js` tampoco fija región. `test-v324-real-traffic.mjs` cubre seis regiones mundiales y `test-v337-universal-weather.mjs` prohíbe sesgo `gt`. `REGISTRO_REINCIDENCIAS_CALIDAD.md` conserva RC-024 abierto hasta Preview y audio físico; Producción permanece intacta.

## V354-R3 · respaldo mundial de clima
