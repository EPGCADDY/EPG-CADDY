...
| Archivo | Función | Protección |
|---|---|---|
| `index-grupal.html` | Valida ronda recuperable, elimina la prioridad de copias vacías, restaura tarjeta archivada y repara la clave canónica. | Jugadores y scores sobreviven cierre, recarga y foco. |
| `test-v365-active-round-empty-recovery.mjs` | Reproducción dinámica de RC-036. | Vacío más reciente no puede vencer a tarjeta operativa. |
| `service-worker.js`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v361-synchronized-voice.mjs` | Renovación V365 del shell con prefijo V363 protegido. | Actualización efectiva en Safari/PWA sin retirar los bancos de voz. |
| `Intocables/README.md`, `Intocables/intocables-gate.mjs`, `test-v363-intocables-behavior.mjs` | INT-01 reforzado. | Persistencia, Match, Normal y voz permanecen acumulativos. |
| `test-v289-stableford-new-round-empty.mjs`, `test-v267-one-operational-line.mjs`, `test-v252-stableford-persistence-category-course.mjs`, `test-v260-round-points-player-return.mjs` | Candados Stableford, arranque, retorno y selector único alineados con la recuperabilidad común. | Un Registro vacío no se trata como ronda operativa, no se duplica el motor y no se mezclan modalidades. |
