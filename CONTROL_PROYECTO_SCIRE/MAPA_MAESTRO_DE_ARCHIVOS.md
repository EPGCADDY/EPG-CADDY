...
|---|---|---|
| `index-grupal.html` | Score natural Safari, jugador pendiente y `AI ∞` de un toque. | Escritor oficial intacto; ambigüedad bloqueada; `voiceOnly` y Cedar conservados. |
| `service-worker.js` | Shell V358. | Actualización efectiva en acceso directo iPhone. |
| `test-v358-ios-score-universal-physical-recovery.mjs` | Banco raíz RC-029. | Hoyo/número/ordinal/golpes, multi-hoyo, jugador operativo, gesto AI y privacidad. |
| `test-v352-live.mjs`, `test-v353-live-hub.mjs`, `test-v354-voice-fallback.mjs`, `test-v355-ios-audio-dictation.mjs`, `test-v356-voice-only-cedar-quality.mjs`, `test-v357-ios-voice-transport-recovery.mjs` | Regresiones actualizadas al build/caché V358. | LIVE y voz V354–V357 no cambian funcionalmente. |
| `.github/workflows/roadmap-gate.yml`, `AGENTS.md`, `audit-project.mjs`, `package.json` | V358 obligatorio en local, GitHub y otras conversaciones. | Un FAIL detiene el candidato. |
| documentos rectores, ambos ROADMAPS, `scripts/rebuild-inventory-pdfs.py` e inventario | RC-029, estado y árbol exacto V358. | Continuidad verificable sin depender del historial de chat. |
