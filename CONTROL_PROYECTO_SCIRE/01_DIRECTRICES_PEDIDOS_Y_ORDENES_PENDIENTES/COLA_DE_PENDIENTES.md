...
La corrección activa ordenada por el propietario es V354 para `PEND-TRA-005` y `PEND-CLI-002`; `PEND-LIVE-018` V353, `PEND-VOZ-003` y los demás pendientes conservan su estado registrado. **Producción permanece intacta y no existe montaje hasta obtener los PASS técnicos y físicos correspondientes.**
...
- V354 interpreta 30 minutos, una hora, tres horas, mañana, próxima semana y fecha/hora ISO en la zona `America/Guatemala`; para minutos intermedios usa la primera hora publicada por Open-Meteo y muestra la resolución de una hora.
- V354 nunca presenta clima actual como pronóstico futuro. Fuera de las 16 jornadas disponibles responde con el rango real y la limitación, sin inventar ni sustituir datos.
...
- V354 extrae salidas en 30 minutos, una hora, tres horas, mañana con hora, próxima semana y fecha ISO; las convierte desde hora Guatemala a ISO, las envía realmente como `departureTime` y rotula la respuesta como prevista, nunca como tráfico en vivo.
- Cuando mañana o próxima semana no incluyen hora, V354 conserva la misma hora local del momento de consulta y declara esa suposición en la respuesta.
