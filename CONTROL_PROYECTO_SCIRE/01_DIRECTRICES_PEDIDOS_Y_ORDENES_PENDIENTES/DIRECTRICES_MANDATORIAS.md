...
- ACTUALIZAR tiene tres puertas independientes y no intercambiables: (1) auditoría automática de código; (2) **REVISIÓN AUTOMATIZADA EN NAVEGADOR REAL** mediante Playwright sobre el alias LAB público y el mismo perfil persistente, con cuatro versiones consecutivas A→B→C→D, capturas completas y evidencia JSON/SHA-256; y (3) prueba física exclusiva en iPhone para el micrófono. Playwright jamás se denomina revisión física. Toda versión permanece `NO REVISADO` hasta completar la segunda puerta sobre deployments READY. Producción permanece intacta.
- El candado `scripts/lab-update-physical-gate.mjs` rechaza evidencia ausente, simulada, incompleta, alterada, de otro commit, deployment o alias, y cualquier recorrido con menos de tres transiciones. Cada transición debe detectar ACTUALIZAR verde/habilitado/parpadeante, tocarlo, confirmar ACTUALIZADO y release final, conservar ronda, Historial, jugador, score y WhatsApp, y registrar cero errores de consola, red, desbordamientos, cortes o traslapes.
