...
assert.match(html,/gscg-build" content="V321-SUSTAINED-CONVERSATION-RECOVERY-20260825"/);
...
assert.match(html,/if\(on\)scheduleConversationIdleClose\(\)/,"Abrir el micrófono debe iniciar únicamente el plazo reglamentario de 30 minutos");
assert.match(html,/if\(listening\)scheduleConversationIdleClose\(\)/,"Cada turno humano debe renovar el plazo de actividad");
assert.match(html,/if\(!realtimeReusableFor\(context\)\)[\s\S]*?await ensureSession\(\)/,"Sólo una sesión dañada debe reconstruirse");
assert.doesNotMatch(html,/Cada nueva activación usa transporte Realtime fresco/,"No debe destruirse una conexión iPhone sana al reabrir");
assert.match(html,/processedTranscriptItems\.clear\(\);processedConversationToolCalls\.clear\(\)/,"Una reconstrucción debe limpiar identificadores obsoletos");
assert.match(html,/NO PUDE COMPLETAR ESA RESPUESTA · PUEDES CONTINUAR/,"Un error de respuesta no debe apagar una conexión sana");
assert.match(html,/setupTranscriptionWatchdog=setTimeout/,"Registro debe salir de TRANSCRIBIENDO si falta el evento final");
assert.match(html,/roundTranscriptionWatchdog=setTimeout/,"Tarjeta debe salir de TRANSCRIBIENDO si falta el evento final");
assert.match(html,/setupMicWrap"\)\.classList\.add\("active"\)/,"La recuperación de Inicio debe conservar visible el micrófono abierto");
assert.match(html,/headerMicWrap"\)\.classList\.add\("active"\)/,"La recuperación de Tarjeta debe conservar visible el micrófono abierto");
...
const req={method:"POST",headers:{host:"golf.test"},body:{query:"Pregunta vigente de prueba"}};
...
console.log("PASS V321 · 24 turnos consecutivos, reapertura sana y fallos web recuperables");
