...
assert.match(html,/async function speakClosure\(text\)[\s\S]*await speakAiUniversalText\(text\)[\s\S]*await ensureSession\(\)[\s\S]*speakAuthorized\("closure",text\)/);
...
const speakClosure=new Function("speakAuthorized","speakAiUniversalText","ensureSession","ensureAnnouncedState","round","persist","clearOperationalMissingPrompt","setVoice","micTrack","getListening","setStopMonitorActive",`${speakClosureSource.replace(/\blistening\b/g,"getListening()").replace(/\bstopMonitorActive=false;/g,"setStopMonitorActive(false);")};return speakClosure`)(speakAuthorized,async()=>false,ensureSession,()=>{}, {announced:{front:true,back:false,complete:false}},()=>{},clearOperationalMissingPrompt,setVoice,micTrack,()=>listening,value=>{stopMonitorActive=value});
...
assert.match(html,/const saveManualHole=\(\)=>\{primeAiUniversalSpeechFromGesture\(\)/,"ENTER debe habilitar el audio dentro del gesto del usuario");
