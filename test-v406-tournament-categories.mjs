...
for(const label of ["A","B","C","D","SENIOR","SUPER SENIOR","FEMENINA"])assert.ok(html.includes(label),`Falta ${label} en Registro`);
assert.match(html,/TOURNAMENT_PRELOAD_CATEGORIES=\{a:"A",b:"B",c:"C",d:"D",senior:"SENIOR",super_senior:"SUPER SENIOR",female:"FEMENINA"\}/,"Registro debe precargar exactamente A B C D Senior Super Senior Femenina");
...
assert.match(hubHtml,/<option value="a">A<\/option><option value="b">B<\/option><option value="c">C<\/option><option value="d">D<\/option><option value="senior">SENIOR<\/option><option value="super_senior">SUPER SENIOR<\/option><option value="female">FEMENINA<\/option>/,"Centro de Torneos debe precargar exactamente las siete categorías");
assert.doesNotMatch(hubHtml,/<option value="championship">CAMPEONATO<\/option>/,"Campeonato no debe aparecer en la precarga visible");
assert.doesNotMatch(hubHtml,/>S\.SENIOR</,"La etiqueta visible debe decir SUPER SENIOR");
