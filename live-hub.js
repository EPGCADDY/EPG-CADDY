(function(root,factory){const api=factory(root);if(typeof module==="object"&&module.exports)module.exports=api;if(root&&root.document)api.start()})(typeof globalThis!=="undefined"?globalThis:this,function(root){
  ...
  function renderCategoryCard(){
    ...
    const ranking=rows.map(item=>'<tr><td>'+escapeHtml(item.rankLabel||"—")+'</td><td><b>'+escapeHtml(item.name)+'</b></td><td>'+escapeHtml(item.player.handicap)+'</td><td><span class="category-chip category-'+escapeHtml(item.tournamentCategory||"none")+'">'+escapeHtml(categoryShortLabel(item.tournamentCategory))+'</span></td><td>'+(item.finished?'FINAL':item.currentHole||'—')+'</td><td>'+item.gross+'</td><td>'+item.net+'</td><td class="'+(item.relativeToPar<0?"under":item.relativeToPar>0?"over":"")+'">'+relation(item.relativeToPar)+'</td></tr>').join("");
