(function(root,factory){const api=factory(root);if(typeof module==="object"&&module.exports)module.exports=api;if(root&&root.document)api.start()})(typeof globalThis!=="undefined"?globalThis:this,function(root){
  ...
  function renderLeaderboard(){
    ...
    wrap.innerHTML='<table class="leader"><thead><tr><th>POS</th><th>JUGADOR</th><th>HOYO ACTUAL</th><th>GROSS</th><th>NETO</th>'+(showPoints?'<th>PUNTOS</th>':'')+'<th>RESULTADO</th><th>CATEGORÍA</th><th>GRUPO</th><th>MODALIDAD</th><th>SEGUIR</th></tr></thead><tbody>'+rows.map(item=>{const key=item.streamId+":"+item.playerId,isFollowed=followed.has(key),progress=item.finished?"FINAL":item.currentHole?String(item.currentHole):"—";return"<tr><td>"+item.rankLabel+"</td><td>"+escapeHtml(item.name)+"</td><td>"+progress+"</td><td>"+item.gross+"</td><td>"+item.net+"</td>"+(showPoints?"<td>"+(item.mode==="universales"?item.universalesPoints:item.mode==="stableford"?item.player.holes.reduce((sum,hole)=>sum+(Number(hole.stablefordPoints)||0),0):"—")+"</td>":"")+"<td class=\""+(item.relativeToPar<0?"under":item.relativeToPar>0?"over":"")+"\">"+relation(item.relativeToPar)+"</td><td><span class=\"category-chip category-"+escapeHtml(item.tournamentCategory||"none")+"\">"+escapeHtml(categoryShortLabel(item.tournamentCategory))+"</span></td><td>"+escapeHtml(item.groupLabel)+"</td><td>"+escapeHtml(modeLabel(item.mode))+"</td><td><button class=\"star\" data-follow-stream=\""+escapeHtml(item.streamId)+"\" data-follow-player=\""+escapeHtml(item.playerId)+"\" aria-label=\"Seguir a "+escapeHtml(item.name)+"\">"+(isFollowed?"★ PERSONA":"＋ PERSONA")+"</button><button class=\"star\" data-follow-group=\""+escapeHtml(item.streamId)+"\" aria-label=\"Seguir grupo "+escapeHtml(item.groupLabel)+"\">＋ GRUPO</button></td></tr>"}).join("")+"</tbody></table>";
