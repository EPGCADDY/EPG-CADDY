(function(root,factory){const api=factory(root);if(typeof module==="object"&&module.exports)module.exports=api;if(root&&root.document)api.start()})(typeof globalThis!=="undefined"?globalThis:this,function(root){
  ...
  function livePlayerTotals(player){const holes=uniquePlayerHoles(player);let gross=0,net=0,relativeToPar=0,universalesPoints=null;for(const hole of holes){if(hole.explicitX)continue;const holeGross=Number(hole.gross),holeNet=Number(hole.net),holePar=Number(hole.par),holeResult=Number(hole.relativeToPar),hasPoints=Number.isFinite(hole.universalesPoints),holePoints=Number(hole.universalesPoints);if(Number.isFinite(holeGross))gross+=holeGross;if(Number.isFinite(holeNet))net+=holeNet;if(Number.isFinite(holeResult))relativeToPar+=holeResult;else if(Number.isFinite(holeNet)&&Number.isFinite(holePar))relativeToPar+=holeNet-holePar;if(hasPoints)universalesPoints=(universalesPoints||0)+holePoints}const currentHole=holes.length?Math.max(...holes.map(hole=>Number(hole.hole))):0;return{holes:holes.length,currentHole,gross,net,relativeToPar,...(universalesPoints===null?{}:{universalesPoints}),finished:holes.length===18}}
  ...
  function buildLeaderboard(streams,filtered=true){
    const players=(filtered?visibleTournamentPlayers(streams):tournamentPlayers(streams)).sort((left,right)=>left.mode==="universales"&&right.mode==="universales"?right.universalesPoints-left.universalesPoints||right.holes-left.holes||fold(left.name).localeCompare(fold(right.name)):left.relativeToPar-right.relativeToPar||right.holes-left.holes||fold(left.name).localeCompare(fold(right.name)));
  ...
  function renderFavorites(){
    const visible=displayStreams(),streams=favoriteStreams(allTournamentStreams()),target=$("hubFavorites"),resolved=resolveFollows(state,streams,externalStreams);if(!target)return;const rankByPlayer=new Map();for(const tournamentMap of tournamentStreams.values())for(const item of buildLeaderboard(tournamentMap,false))rankByPlayer.set(item.streamId+":"+item.playerId,item.rank);for(const item of buildLeaderboard(visible,false))rankByPlayer.set(item.streamId+":"+item.playerId,item.rank);
  ...
  function renderLeaderboard(){
    ...
    wrap.innerHTML='<table class="leader"><thead><tr><th>POS</th><th>JUGADOR</th><th>CATEGORÍA</th><th>GRUPO</th><th>MODALIDAD</th><th>HOYO ACTUAL</th><th>GROSS</th><th>NETO</th>'+(showPoints?'<th>PUNTOS</th>':'')+'<th>RESULTADO</th><th>SEGUIR</th></tr></thead><tbody>'+rows.map(item=>{const key=item.streamId+":"+item.playerId,isFollowed=followed.has(key),progress=item.finished?"FINAL":item.currentHole?String(item.currentHole):"—";return"<tr><td>"+item.rankLabel+"</td><td>"+escapeHtml(item.name)+"</td><td><span class=\"category-chip category-"+escapeHtml(item.tournamentCategory||"none")+"\">"+escapeHtml(categoryShortLabel(item.tournamentCategory))+"</span></td><td>"+escapeHtml(item.groupLabel)+"</td><td>"+escapeHtml(modeLabel(item.mode))+"</td><td>"+progress+"</td><td>"+item.gross+"</td><td>"+item.net+"</td>"+(showPoints?"<td>"+(item.mode==="universales"?item.universalesPoints:item.mode==="stableford"?item.player.holes.reduce((sum,hole)=>sum+(Number(hole.stablefordPoints)||0),0):"—")+"</td>":"")+"<td class=\""+(item.relativeToPar<0?"under":item.relativeToPar>0?"over":"")+"\">"+relation(item.relativeToPar)+"</td><td><button class=\"star\" data-follow-stream=\""+escapeHtml(item.streamId)+"\" data-follow-player=\""+escapeHtml(item.playerId)+"\" aria-label=\"Seguir a "+escapeHtml(item.name)+"\">"+(isFollowed?"★ PERSONA":"＋ PERSONA")+"</button><button class=\"star\" data-follow-group=\""+escapeHtml(item.streamId)+"\" aria-label=\"Seguir grupo "+escapeHtml(item.groupLabel)+"\">＋ GRUPO</button></td></tr>"}).join("")+"</tbody></table>";
  ...
  async function refresh(){
    ...
    if(result&&result.ok)setStatus(demoMode()?"TORNEO DE DEMOSTRACIÓN · DATOS DE EJEMPLO":state.generalToken?"MONITOR DEL TORNEO ACTUALIZADO · RESULTADOS + SEGUIMIENTO":state.follows.length?"FAVORITOS ACTUALIZADOS":"AGREGA UN TORNEO O PEGA UN ENLACE PRIVADO","");
  ...
  function showMonitor(kind){
    ...
    $("hubGeneralPanel")?.classList.toggle("hidden",tournamentPortalOpen||individual);
    $("hubIndividualPanel")?.classList.toggle("hidden",tournamentPortalOpen||!individual);
