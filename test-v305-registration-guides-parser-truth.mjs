...
for(const text of ["DICTA ASÍ:","NOMBRE · 14 · BLANCAS","NOMBRE · 21 · AZULES","HASTA 6 JUGADORES","LUEGO TOCA OK"])assert.ok(html.includes(text),`Falta guía General: ${text}`);
...
assert.match(stable,/Registro Stableford Scratch[\s\S]*?Jugador 1 NOMBRE; Jugador 2 NOMBRE; hasta Jugador 6/);
assert.match(html,/function generalSetupState\(\)[\s\S]*?complete&&count>=1&&course\?\.configured/);
