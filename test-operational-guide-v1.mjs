...
for(const field of ['El Pulté','Country Club','San Isidro','Mayan Golf','Hacienda Nueva','Alta Vista','La Reunión']){
  if(!source.includes(field))fail(`falta campo universal: ${field}`);
}
if(/\b(?:Jaime|Roberto|Ana|Eduardo)\b/i.test(source))fail('la guía conserva nombres reales en ejemplos');
if(source.includes('OUT, IN'))fail('la guía conserva el orden OUT, IN');
if(!builder.includes('w:val')||!builder.includes('for side in ("top", "bottom", "left", "right")')){
  fail('los títulos neón no conservan fondo y marco negro');
}
