...
// Las pruebas funcionales históricas inspeccionan index-grupal.html. Durante el
// audit se materializa allí la fuente canónica completa; apply-update-e.mjs la
// transforma nuevamente en el cargador persistente después de aprobar todo.
const canonicalHtml=readFileSync('candidate-index-grupal.html','utf8');
if(canonicalHtml.length<300000||!canonicalHtml.includes('id="setupMic"')){
  throw new Error('CANONICAL_AUDIT_SOURCE_INVALID');
}
writeFileSync('index-grupal.html',canonicalHtml);
console.log(`AUDIT_SOURCE PASS candidate-index-grupal.html -> index-grupal.html chars=${canonicalHtml.length}`);
...
checks.splice(checks.indexOf('test-score-engine.mjs'),0,'test-v398-manual-opening-hole.mjs');
checks.splice(checks.indexOf('test-score-engine.mjs'),0,'test-v398-history-long-press-delete.mjs');
