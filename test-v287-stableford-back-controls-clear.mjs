...
assert.match(html,/<div class="round-secondary-actions" id="roundSecondaryActions"><button class="back-registration-button" id="backToRegistrationButton"[\s\S]*?>ATRÁS<\/button>[\s\S]*?<button class="clear-round-scores" id="clearScoresOnly"[\s\S]*?>BORRAR SCORES<\/button>[\s\S]*?<button class="clear-round-scores" id="clearRoundScores"[\s\S]*?>BORRAR TODO<\/button><button class="back-registration-button add-player-button/);
