...
export const RELEASE_URL="/update-lab/release.json";
export const SW_URL="/update-lab/update-sw.js";

export async function fetchRelease(){
  const response=await fetch(`${RELEASE_URL}?t=${Date.now()}`,{cache:"no-store",headers:{"cache-control":"no-cache"}});
  if(!response.ok)throw new Error(`RELEASE_HTTP_${response.status}`);
  const release=await response.json();
  if(release?.schema!=="gscg-update-release/v2"||!release.release)throw new Error("RELEASE_INVALID");
  return release;
}
...
export function queryActiveRelease(worker=navigator.serviceWorker.controller,timeoutMs=5000){
  return new Promise((resolve,reject)=>{
    ...
    channel.port1.onmessage=event=>{clearTimeout(timer);const value=String(event.data?.release||"");value?resolve(value):reject(new Error("ACTIVE_RELEASE_EMPTY"))};
