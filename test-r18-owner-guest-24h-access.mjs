...
assert.match(api,/action==="redeem"&&req\.method==="POST"/);
assert.doesNotMatch(api,/action==="redeem"&&req\.method==="GET"/);
assert.match(api,/redeemGuestToken\(token\)/);
assert.match(api,/YA UTILIZADO/);
assert.match(api,/\/invite\/\$\{encodeURIComponent\(grant\.token\)\}/);
...
assert.match(app,/text:`Invitación individual[^`]*\\n\$\{data\.url\}`/);
assert.match(app,/navigator\.share\(payload\)/);
assert.match(app,/\.round-utility-bar #ownerShare24h/);
