...
assert.match(app,/access\?\.role==="owner"&&access\?\.canShare===true/);
assert.match(app,/fetch\("\/api\/app-access\?action=create",\{method:"POST",credentials:"include"\}\)/);
assert.match(app,/navigator\.share\(shareData\)/);
assert.match(app,/registrar de uno a seis jugadores y anotar los scores por voz o escritura/);
assert.match(app,/id="ownerShare24hSetup"[^>]*>WHATSAPP · INVITAR 24H<\/button>/);
assert.match(app,/REGISTRA SÓLO A UNO O AL GRUPO · HASTA 6/);
assert.match(app,/ownerShare24hButtons\.forEach\(button=>button\.addEventListener\("click",shareOwnerAccess24h\)\)/);
