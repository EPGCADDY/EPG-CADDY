...
assert.match(source,/async function png\(item\)\{const canvas=await canvasFor\(item\);return canvasBlob\(trimCanvas\(canvas,20\),"image\/png"\)\}/,"PNG compartido debe recortar el margen negro sobrante después del render Full HD");
