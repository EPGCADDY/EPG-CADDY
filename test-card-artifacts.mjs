...
const out=artifacts.build(snapshot);assert.equal(out.personal.length,1);assert.equal(out.all.length,2);assert.doesNotMatch(out.global.html,/>Tarjeta Global</);assert.match(out.global.html,/golf-score-card-gt-horizontal-original\.webp/);assert.match(out.global.html,/CAMPO · El Pulté Golf/);assert.match(out.global.html,/MODALIDAD · MEDAL PLAY NORMAL/);assert.match(out.global.html,/FECHA ·/);assert.doesNotMatch(out.global.html,/VERSIÓN ·|ID OFICIAL · SHA-256/);assert.match(out.personal[0].html,/Comportamiento Neto contra Par/);assert.match(out.personal[0].html,/Águilas/);assert.match(out.personal[0].html,/SHA-256/);assert.throws(()=>artifacts.build({...snapshot,status:'active'}));console.log('PASS archivos Global/personal desde snapshot oficial');
