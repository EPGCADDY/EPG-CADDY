...
const out=artifacts.build(snapshot);assert.equal(out.personal.length,1);assert.equal(out.all.length,2);assert.doesNotMatch(out.global.html,/>Tarjeta Global</);assert.ok(out.global.html.includes('golf-score-card-gt-horizontal-original.webp'));
