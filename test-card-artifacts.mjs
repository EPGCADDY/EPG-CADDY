...
// LAB R127 final visual/format contract
{
 ...
 const mp=artifacts.build({...snapshot,mode:'match_play',players:optionalCategoryPlayers.slice(0,4),matchPlay:{resultLabel:'MATCH PLAY'}}).global.html;
 ...
 for(const p of optionalCategoryPlayers.slice(0,4)) assert.match(mp,new RegExp(p.name.split(' ')[0]),'Deben aparecer exactamente los cuatro jugadores de las dos parejas');
}
