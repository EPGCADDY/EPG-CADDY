...
const fiveHoleBatch=parse('hoyo uno cuatro hoyo dos cinco hoyo tres cuatro hoyo cuatro seis hoyo cinco tres',{defaultHole:1,defaultPlayer:players[0]});
assert.equal(fiveHoleBatch.ok,true);
assert.deepEqual(fiveHoleBatch.entries.map(e=>[e.player,e.hole,e.gross]),[["JAIME",1,4],["JAIME",2,5],["JAIME",3,4],["JAIME",4,6],["JAIME",5,3]]);
console.log('PASS five explicitly dictated holes retain all five scores for Jaime');
