...
export default async function accessGate(request){
  ...
  try{const statusUrl=new URL("/api/app-access?action=status",request.url),response=await fetch(statusUrl,{headers:{cookie:request.headers.get("cookie")||""},cache:"no-store"}),data=await response.json();access={ok:response.ok&&data.ok===true,role:data.role||"none",code:data.code||null}}catch{}
  if(access.ok){
