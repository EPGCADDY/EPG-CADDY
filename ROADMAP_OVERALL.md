...
La captura física `IMG_2187.png` y los logs de Producción a las 22:37 UTC rechazaron el enlace como definitivo: Safari escuchó, `/api/universal-ai` respondió 200, pero Cedar recibió 429, `/api/voice-speech` devolvió 503 y no existía un locutor local aceptado. RC-036 registra el punto de escape: el banco simulaba Gateway autorizado y sólo comprobaba un nombre masculino español.

V365 conserva íntegros tarjeta, escritor, persistencia, Match Play, Ronda Normal y LIVE. La voz encadena Cedar `gpt-4o-mini-tts` → Onyx directo `tts-1` → Gateway oficial `openai/tts-1` → locutor masculino local Safari precargado y ampliado. El respaldo local admite un hombre aprobado fuera del idioma exacto, pero continúa rechazando voces femeninas genéricas. `test-v365-universal-male-voice-recovery.mjs` reproduce Cedar 429 sin Gateway, recuperación Onyx directa y los catálogos Reed/Mónica/Samantha.

Archivos exactos V365: `api/voice-speech.js`, `api/voice-health.js`, `index-grupal.html`, `service-worker.js`, `test-v356-voice-only-cedar-quality.mjs`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v361-synchronized-voice.mjs`, `test-v362-physical-voice-recovery.mjs`, `test-v365-universal-male-voice-recovery.mjs`, `audit-project.mjs`, `package.json`, `Intocables/README.md`, `Intocables/REGLAS_INTOCABLES.json`, `Intocables/intocables-gate.mjs`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `CONTROL_PROYECTO_SCIRE/03_CASOS_TERMINADOS_Y_EVIDENCIA/V365_UNIVERSAL_VOICE_RECOVERY/REPORTE_V365_RC036.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Candidato sujeto a auditoría integral, Preview y PASS físico iPhone; no se declara definitivo.
