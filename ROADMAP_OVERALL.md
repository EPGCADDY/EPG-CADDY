...
## V366 · conversación universal sólo audio · 28 de agosto de 2026

La evidencia física `IMG_2193(1).png` cerró el criterio operacional: una pregunta hablada desde la tarjeta, Registro o cualquier modalidad no navega a AI UNIVERSAL. La pantalla actual permanece visible y la respuesta se entrega únicamente por audio. El defecto provenía de dos contratos heredados que abrían el panel antes de responder y antes de escuchar, aun cuando `voiceOnly` ya ocultaba ambos turnos.

V366 reemplaza esas dos aperturas por `closeAiUniversalPanel()`, conserva `visible:false`, mantiene el panel explícito de Reglas y registra telemetría privada `browser_fallback_general_audio_only`. `test-v366-universal-audio-only-context.mjs` bloquea la reaparición de otra pantalla; V354 y V358 fueron ajustados al nuevo contrato sin tocar scores, persistencia, Match Play, Four Ball ni Ronda Normal.

Archivos exactos V366: `index-grupal.html`, `api/voice-health.js`, `service-worker.js`, `test-v354-voice-fallback.mjs`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v358-ios-score-universal-physical-recovery.mjs`, `test-v361-synchronized-voice.mjs`, `test-v366-universal-audio-only-context.mjs`, `audit-project.mjs`, `package.json`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`, `ROADMAP_OVERALL.md` y `ROADMAP_A_DETALLE.md`. Producción permanece en `194a4af`; Preview, tres comprobaciones externas y prueba física iPhone son puertas separadas.
