...
La prueba física detectó voz femenina porque R7 fijaba modelo, idioma, velocidad e instrucciones, pero no un identificador de locutor. V383 añade en `api/voice-speech.js` el ID masculino fijo `f0325cd11aac4fa983eb41ca2d371660`; `test-v378-approved-r7-voice-lock.mjs`, `test-v383-fixed-male-voice.mjs` y `test-v362-physical-voice-recovery.mjs` exigen modelo Fish, voice ID, `es-419` y `0.90` exactos. `service-worker.js`, `audit-project.mjs`, `package.json`, RC-063, ambos ROADMAPS e inventario impiden volver a una selección variable. V382 no se entrega por voz rechazada. Registro, Scores y Producción permanecen intactos.
