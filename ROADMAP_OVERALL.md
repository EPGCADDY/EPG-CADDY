...
### V373 · captura estable y transcripción española separada

La comprobación adicional confirmó `403 GatewayForbiddenError` también para `openai/gpt-realtime-mini`, aun con US$21.36 visibles en AI Gateway. V373 elimina esa dependencia: iPhone graba cada turno con `MediaRecorder`, cierra físicamente la pista después de un segundo de silencio y envía sólo ese audio a `api/voice-transcribe.js`; el texto español vuelve a la frontera existente `processBrowserVoiceTranscript()`. `api/_lib/traffic.js` deriva además el código de país desde las coordenadas GPS y lo entrega a Google Routes, evitando que `Oakland Mall zona 10` se resuelva en Estados Unidos cuando el origen está en Guatemala; `test-v324-real-traffic.mjs` fija `regionCode="GT"`. Registro, parsers de Score, cálculos, persistencia, interfaz, clima y Producción permanecen sin cambios funcionales. Evidencia: `REPORTE_V373_RC042.md` y `test-v373-server-voice-transcription.mjs`.
...
V374-R1 registra la prueba viva del token OpenAI: el Preview READY devolvió `403` aun con `routeKind=transcription`. El endpoint aplica entonces failover únicamente entre los otros dos modelos de transcripción transmitida publicados por AI Gateway: `google/gemini-3.5-transcribe-live` y `spacexai/grok-stt`. No cambia cliente, parser, escritor, interfaz ni datos.

V374-R2 localiza el 403 en la autenticación automática OIDC del método experimental de token: las llamadas ordinarias del Gateway sí funcionaban, pero los tres tokens de transcripción eran rechazados. Se crea una clave AI Gateway restringida a `lab/recovery-production-approved-v363` y se agrega un `GET ?health=stream-token` disponible exclusivamente fuera de Producción; la comprobación consume y descarta el token, informa modelo y latencia, y nunca devuelve la credencial. Archivos modificados: `api/voice-transcribe.js`, `test-v373-server-voice-transcription.mjs`, `scripts/rebuild-inventory-pdfs.py`, ambos ROADMAPS, reporte RC-042, registro de reincidencias e inventario. Registro, Score y Producción permanecen intactos.
...
La revisión de la evidencia preservada confirmó que R7 fue la voz aprobada físicamente: `fish-audio/s2.1-pro-free`, español latinoamericano `es-419` y velocidad `0.90`. El defecto no fue una pérdida de esos parámetros, sino una ruta no determinista: `api/voice-speech.js` intentaba primero Onyx y sólo llegaba a R7 cuando ese proveedor fallaba; el cliente podía sustituirla otra vez por una voz local de Safari. V378 convierte R7 en la única salida audible. Si no está disponible, muestra un error explícito y no cambia el timbre. `test-v378-approved-r7-voice-lock.mjs` ejecuta el endpoint y bloquea cualquier llamada directa o respaldo local. Producción permanece intacta; el Preview y el PASS audible del iPhone siguen siendo puertas separadas.
...
## V381 · reinicio serial de salida antes del siguiente micrófono · 4 de septiembre de 2026

La prueba física rechazó V380 en la segunda pregunta: conservar el reproductor descargado evitó otro primer silencioso, pero la sesión de salida de iOS siguió retenida y SpeechRecognition terminó `no_speech/aborted`. V381 cambia exclusivamente la transición posterior a una respuesta Universal en `index-grupal.html`: dentro del siguiente toque destruye la salida anterior, autoriza un reproductor nuevo, lo descarga y espera 300 ms antes de abrir el micrófono. `test-v381-serial-audio-input-reset.mjs`, `test-v380-three-turn-audio-release.mjs`, `test-v355-ios-audio-dictation.mjs`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v361-synchronized-voice.mjs`, `service-worker.js`, `audit-project.mjs` y `package.json` fijan el orden y admiten cachés posteriores sin debilitar sus controles funcionales. Registro y Scores V378 continúan sellados por SHA-256; la voz R7 y Producción permanecen intactas.

## V382 · reinicio exclusivo de entrada sin audio silencioso · 4 de septiembre de 2026

V381 fue rechazada físicamente: la primera pregunta se transcribió y habló en 1.829 s, pero el audio silencioso de recuperación volvió a ocupar la sesión iOS y la segunda captura terminó `no_speech/aborted`. V382 elimina completamente ese cebado entre turnos. `index-grupal.html` destruye y descarga la salida anterior, espera 300 ms y abre únicamente SpeechRecognition; además registra ambos extremos del reset. `test-v382-input-only-reset.mjs` y `test-v381-serial-audio-input-reset.mjs` prohíben reintroducir `new Audio()`, `play()` o WAV silencioso en esa transición. `service-worker.js`, `audit-project.mjs`, `package.json`, ambos ROADMAPS, RC-062 e inventario fijan V382. Registro, Scores, voz R7 y Producción permanecen intactos.

## V383 · locutor masculino fijo y no interpretable · 4 de septiembre de 2026

La prueba física detectó voz femenina porque R7 fijaba modelo, idioma, velocidad e instrucciones, pero no un identificador de locutor. El primer ID fijo de V383 fue rechazado físicamente por timbre ahogado; V385 lo sustituye por `06ddea79e86a412aa4babdd69917331a`, voz Fish pública descrita como masculina, clara, natural y conversacional. `test-v378-approved-r7-voice-lock.mjs`, `test-v383-fixed-male-voice.mjs` y `test-v362-physical-voice-recovery.mjs` exigen modelo Fish, voice ID, `es-419` y `0.90` exactos. Registro, Scores y Producción permanecen intactos.

## V384 · blindaje completo de la activación compartida · 4 de septiembre de 2026

La confirmación V378 prometía preservar Registro y Scores, pero el sello inicial cubría seis regiones y omitía `fireMicActivation()`, punto común que V381/V382 alteraron sin disparar el candado. V384 restaura esa función byte por byte desde V378 y agrega su SHA-256 como séptima región obligatoria. La limpieza posterior del audio Universal queda fuera del gesto compartido y no agrega esperas, reproducción silenciosa ni estados de reset antes de `toggleVoice()`. El control se ejecuta en `Intocables/intocables-gate.mjs` y `test-v384-full-input-chain-lock.mjs`; se actualizan `index-grupal.html`, `Intocables/APROBACION_FISICA_REGISTRO_SCORES_V378.json`, `Intocables/CONFIRMACION_ESCRITA_V378.md`, `test-v355-ios-audio-dictation.mjs`, `test-v381-serial-audio-input-reset.mjs`, `test-v382-input-only-reset.mjs`, `service-worker.js`, `audit-project.mjs`, `package.json`, `scripts/rebuild-inventory-pdfs.py`, RC-064, ambos ROADMAPS, mapa maestro e inventario. Producción permanece intacta.

## V385 · transición explícita de sesión de audio iOS · 4 de septiembre de 2026

La prueba física V384 reconoció y respondió la primera consulta, pero la siguiente captura terminó `no_speech/aborted`. Los logs demostraron que el audio Fish finalizó y Safari conservó la categoría de salida. V385 usa `navigator.audioSession`: `transient-solo` durante la respuesta y `play-and-record` antes de cada reconocimiento, sin modificar las siete regiones V378. Archivos: `index-grupal.html`, `service-worker.js`, `test-v385-ios-audio-session.mjs`, `test-v355-ios-audio-dictation.mjs`, `audit-project.mjs`, `package.json`, `scripts/rebuild-inventory-pdfs.py`, RC-065, ambos ROADMAPS, mapa maestro e inventario. Producción intacta.
## V390 · recuperación física de Historial y segundo turno Universal · 5 de septiembre de 2026

La prueba física V389 rechazó nuevamente el producto: el Historial apareció vacío y el segundo turno terminó en `RECONOCIMIENTO DE VOZ NO DISPONIBLE`. Los logs exactos de `dpl_9v8z35uczWbmM7KM5Tg3CY4kAFrf` muestran primera transcripción, `/api/universal-ai` 200 y voz 200; veinte segundos después el segundo `SpeechRecognition` abrió, agotó ocho segundos y cerró con `browser_fallback_no_result_timeout` + `no_speech/aborted`.

V390 conserva el reproductor autorizado pero descargado después de la primera voz, evitando otro audio silencioso en el siguiente gesto, y obliga a que el seguimiento Universal post-TTS use una captura PCM aislada que sólo puede responder preguntas: nunca escribe Registro ni Scores. La ruta, sensibilidad, umbrales, cierre y activación V378 continúan byte por byte intactos. Los eventos `server_capture_started`, `server_capture_transcript_ready` y `server_capture_failed` quedan observables. El Historial deja de depender de una promesa posterior al arranque: `round-closure.js` incorpora cierre síncrono con SHA-256 real y la ronda oficial Stableford del 4 de septiembre se firma y archiva antes de continuar, de manera idempotente y sin sustituir una ronda activa.

La evidencia física adicional confirma que la voz también cambió. La comparación contra el commit aprobado V378 demostró que V383 agregó un `voice` ID que no existía en el payload aprobado. V390 retira esa reinterpretación y restaura `api/voice-speech.js` exactamente al blob V378 (`88d493a`, SHA-256 `dd4597f6…5fee`), incluida velocidad `0.90`; el candado ahora protege el archivo completo.

Control permanente: `test-v390-physical-history-voice-recovery.mjs`. Archivos: `api/voice-speech.js`, `round-closure.js`, `index-grupal.html`, `service-worker.js`, `Intocables/APROBACION_FISICA_REGISTRO_SCORES_V378.json`, `Intocables/CONFIRMACION_ESCRITA_V378.md`, `Intocables/REGLAS_INTOCABLES.json`, `Intocables/intocables-gate.mjs`, pruebas V357/V361/V362/V376/V378/V379/V381/V382/V383/V385/V386/V388/V390, `audit-project.mjs`, `package.json`, reincidencias, matriz/cola, mapa maestro, ambos ROADMAPS, reconstrucción e inventario. Producción permanece intacta; el estado seguirá **pendiente de prueba física** hasta voz V378 confirmada, Historial visible y tres preguntas consecutivas habladas en el mismo iPhone.
...
## V392 · segundo turno Universal e IN antes de OUT en todas las tarjetas · 5 de septiembre de 2026

La prueba física V391 queda rechazada: los logs reales del mismo iPhone muestran dos pares consecutivos `server_capture_started` → `server_capture_failed: no_speech`. La pista sí se adquiría, pero el procesador Web Audio no recibió muestras después de la primera respuesta. V392 graba el audio Universal directamente desde la pista con `MediaRecorder` (`audio/mp4`) y usa Web Audio únicamente para detectar actividad con el mismo umbral de Registro: `0.009`. Aunque el analizador iOS permanezca en cero, la ventana fija de seis segundos conserva y transcribe el archivo real. Registro, Scores y la voz V378 a velocidad `0.90` permanecen intactos.

La convención obligatoria queda unificada: **IN = primera vuelta, hoyos 1–9; OUT = segunda vuelta, hoyos 10–18; IN siempre aparece primero**. Se corrigieron tarjeta principal y final, Ronda Normal, Práctica, Stableford, Match Play, Four Ball, control manual, resúmenes, tarjeta Global/personal descargable, Historial, LIVE y la tarjeta independiente `stableford-torneo.html`. Los valores front/back se reasociaron con sus rótulos correctos; no es un simple intercambio visual.

`test-v392-universal-mediarecorder.mjs` ejecuta tres capturas consecutivas en una sola instancia. `test-v392-all-scorecards-in-first.mjs` inventaría todas las superficies y valida orden y totales 1–9/10–18. INT-12 e INT-13 incorporan ambas reglas al gate Intocables. Producción permanece intacta. Estado: controles técnicos en curso; Comunicación Universal sigue **pendiente de prueba física 3/3** hasta probar el Preview en el mismo iPhone.

## V393 · Control Manual y respuesta Universal continua · 5 de septiembre de 2026

La prueba física V392 queda rechazada. El elemento de Control Manual conservaba `dataset.hole=18` al sustituir una ronda por otra dentro del mismo documento; V393 lo vincula al ID de ronda y, al cambiar, selecciona el hoyo 1 si no existen scores o el primer hoyo pendiente si ya existen. Aplica igualmente a General, Práctica, Stableford, Match Play y Four Ball.

Los logs reales V392 muestran transcripción correcta y luego `/api/universal-ai` 502 tanto en Inicio como en tarjeta. El backend aceptaba únicamente `output[].content[].type=output_text`; el Gateway puede devolver respuestas válidas como `text` o `output_text` directo. V393 normaliza los formatos válidos sin tocar voz V378, velocidad 0.90, Registro ni Scores. `test-v393-manual-hole-start.mjs` y `test-v393-universal-response-continuity.mjs` quedan bloqueantes. Producción permanece intacta; Universal sigue pendiente de prueba física 3/3.

Trazabilidad V393: también se actualizan `service-worker.js`, `audit-project.mjs`, `package.json`, `Intocables/REGLAS_INTOCABLES.json`, `Intocables/intocables-gate.mjs`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `scripts/rebuild-inventory-pdfs.py` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`.

## V394 · cierre correcto del archivo de audio iOS · 5 de septiembre de 2026

V393 queda rechazada por la prueba física y los logs del deployment `dpl_4rALbgNEtBMtaFpDdcdNA6hnzy3i`: la primera interacción habló, pero las dos capturas posteriores terminaron `server_capture_failed/no_speech` sin llamar a `/api/voice-transcribe`. La causa queda aislada en `server-voice-capture.js`: el banco simulaba que `requestData()` entregaba el blob antes de `stop()`, mientras Safari dejó la colección vacía.

V394 usa `MediaRecorder.start()` sin `timeslice` ni `requestData`, procesa el blob final y limpia el contador de recuperación. `index-grupal.html` conserva el reconocimiento nativo primero y el aislamiento de Universal; `service-worker.js` fuerza el recurso nuevo. `test-v392-universal-mediarecorder.mjs` reproduce tres cierres consecutivos con el orden real `dataavailable` → `stop` y prohíbe las dos llamadas causales. No cambian cálculos, persistencia, diseño, navegación, voz R7, velocidad 0.90 ni Producción.

Control de publicación: el primer commit V394 quedó rechazado por transferencia remota incompleta de `index-grupal.html`; el blob íntegro quedó comprobado por SHA Git `3cf766c23fc54ba2c17107793e18b7fb0c457b84`. El commit de cierre debe incluir ambos ROADMAPS y el sello de inventario junto al árbol completo; ningún deployment incompleto se entrega.
