...
## V351-R1 · Micrófono individual escribe y muestra el score desde el hoyo 1 · 27 de agosto de 2026

Se recuperó el fallo exacto anterior a LIVE: en una ronda con un solo jugador, la transcripción `hoyo uno, cuatro` no tenía jugador implícito antes del primer recordatorio; podía salir del flujo sin escribir ni mostrar el Gross. `index-grupal.html` selecciona ahora exclusivamente al único jugador activo, conserva nombre obligatorio cuando existen dos o más jugadores y mantiene silenciosa la confirmación del reconocimiento alternativo. `test-v351-r1-hole1-voice-score-render.mjs` ejecuta la frase, el escritor oficial, persistencia y render de la celda del hoyo 1; `test-v270-consecutive-hole-voice-blocks.mjs` protege la frontera grupal. `audit-project.mjs` incorpora la puerta 95. RC-024 conserva el defecto y la prueba física iPhone pendiente.

Archivos exactos: `index-grupal.html`, `test-v351-r1-hole1-voice-score-render.mjs`, `test-v270-consecutive-hole-voice-blocks.mjs`, `audit-project.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Los tres artefactos restaurados son `Inventario_Golf_Score_Card_GT_OVERALL_V311.pdf`, `Inventario_Golf_Score_Card_GT_A_DETALLE_V311.pdf` e `Inventario_Golf_Score_Card_GT_POR_IMAGENES_Y_RUBROS_V311.pdf`. Producción permanece intacta hasta cero FAIL y aprobación física.

## V351-R2 · Apertura del micrófono dentro del toque de Safari/iOS · 27 de agosto de 2026

Los registros del deployment V351 rechazado confirmaron HTTP 429 en `/api/session-grupal`. En iPhone, esperar ese fallo antes de iniciar el reconocimiento alternativo hacía que el gesto de usuario ya no fuera vigente y el micrófono pudiera cerrarse sin escuchar. `index-grupal.html` detecta Safari/iOS y abre `SpeechRecognition`/`webkitSpeechRecognition` sincrónicamente dentro del toque, antes de cualquier `await`; Registro y Score conservan los mismos parsers, escritor oficial, persistencia y render. `test-v336-microphone-transport.mjs` exige el orden gesto → reconocimiento → cualquier sesión remota. El corte de inventario pasa a `V351-R2-IOS-TOUCH-HOLE1-RENDER`; Producción sigue intacta y la prueba física iPhone continúa siendo la puerta final.

El mismo corte corrige la falla visual del clima: el texto móvil anterior era de 7.7 px. `index-grupal.html` impone 10.5 px y 40 px de altura mínima en la Score Card, y 11 px/44 px en Registro. `test-v312-general-caddie.mjs` bloquea ambos mínimos sin cambiar la fuente Open-Meteo ni el refresco automático.

La primera corrida integral detectó que `test-v267-scorecard-combination-matrix.mjs` seguía fijado al contrato anterior `operationalPromptPlayer()`. La prueba se migra al contrato V351-R2 `operationalDefaultPlayer(defaultHole)` y exige simultáneamente la condición negativa `active.length === 1`; no cambia el motor, sólo alinea la puerta histórica con la corrección aprobada.

## V351-R3 · Score local inviolable después de la transcripción · 27 de agosto de 2026

La prueba física de las 17:12 rechazó V351-R2: Registro sí funcionó y el Score recibió dos transcripciones, pero ninguna escribió Gross. Los logs demostraron que ambas escaparon a `/api/universal-ai`, que respondió 503 por saldo agotado. El defecto no estaba en el micrófono sino en un blindaje incompleto que probaba piezas separadas sin ejecutar la frontera micrófono alternativo → clasificador → Score con dos jugadores.

`index-grupal.html` procesa ahora una anotación válida antes que una consulta y conserva las preguntas explícitas sin escritura. Toda frase que pertenezca al Score termina localmente: si es válida usa `applyLiteralScores`; si falta nombre o formato muestra una instrucción visible y no consulta IA. `test-v351-r1-hole1-voice-score-render.mjs` ejecuta `Jaime hizo cuatro y Jorge hizo cinco`, el enrutador real y el micrófono alternativo; además reproduce `hoyo uno cuatro` con dos jugadores y exige cero llamadas a `submitAiUniversalText`. `test-v336-microphone-transport.mjs` y la telemetría privada separan aplicado/rechazado sin guardar transcripción, nombres ni scores. Producción permanece intacta; faltan auditoría integral, Preview nuevo y confirmación física iPhone.

Archivos exactos V351-R3: `index-grupal.html`, `api/voice-health.js`, `test-v351-r1-hole1-voice-score-render.mjs`, `test-v336-microphone-transport.mjs`, `test-v267-scorecard-combination-matrix.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`.

## V351-R4 · Score Match Play compatible con la transcripción numérica de Safari · 28 de agosto de 2026

La prueba física de las 18:21 rechazó V351-R3: Registro de Match Play aplicó correctamente a Jaime y Gustavo, la transcripción del Score llegó, pero terminó en `browser_fallback_score_rejected` y ambos Gross quedaron vacíos. El antecedente XIV confirmó que Safari puede representar números hablados como romanos; el parser de Gross aún no cubría IV/V y tampoco aceptaba las unidades naturales `golpes/tiros`.

`index-grupal.html` agrega conversión romana canónica I–XXX sólo en el lector de Gross. `X` se excluye expresamente y continúa significando sin dato. La prueba V351-R4 reproduce Match Play con `Jaime IV Gustavo V`, escribe Gross 4/5 mediante el escritor oficial, persiste, renderiza y cuenta cero llamadas a IA. `test-v305-registration-guides-parser-truth.mjs` protege el alcance Registro/Score/X.

Archivos exactos V351-R4: `index-grupal.html`, `test-v351-r1-hole1-voice-score-render.mjs`, `test-v305-registration-guides-parser-truth.mjs`, `test-v336-microphone-transport.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece intacta.

## V351-R5 · Score por voz en todas las modalidades y cantidades admitidas · 28 de agosto de 2026

La prueba física de las 18:46 rechazó V351-R4. Registro aplicó los dos jugadores; Score inició el reconocimiento, recibió una transcripción y emitió `browser_fallback_score_rejected`; ambos Gross permanecieron vacíos. La telemetría privada no conserva texto hablado, nombres ni scores, por lo que no se inventa una palabra causal. El escape comprobado fue de cobertura: las 583 combinaciones históricas validaban estructura, mientras R4 sólo ejecutaba el micrófono completo con dos jugadores en Match Play.

V351-R5 conserva el parser estricto y agrega una recuperación segura por segmentos nombrados. Acepta un Gross inequívoco por jugador y un hoyo válido antes o después de la tanda, tolera palabras intermedias sin adivinar jugador ni score, rechaza duplicados y preguntas, y continúa utilizando el escritor oficial. Los rechazos sólo pueden emitir códigos cerrados de etapa sin datos privados.
...
## V351-R8 · cola de Safari completa y preguntas visibles · 28 de agosto de 2026

La prueba física de las 23:15 rechazó R7. Los logs del deployment `dpl_3MsjXDJeQjdqtPKe9H6VZhxLDHdN` muestran `ambiguous_score` a las 05:15:39 UTC y un score individual aplicado a las 05:15:54. Dos preguntas posteriores sí recibieron HTTP 200 de `/api/universal-ai` a las 05:16:05 y 05:16:15, pero el usuario no vio respuesta.

R8 corrige dos puntos verificables. Primero, Safari puede terminar una frase larga con un fragmento final y una cola interina; `mergeBrowserVoiceSegments` conserva ambos y elimina únicamente el solapamiento, en vez de descartar la cola mediante `final || interim`. Segundo, toda pregunta hecha con el micrófono de la Score Card abre AI UNIVERSAL visiblemente sin levantar el teclado; el texto queda disponible aunque iOS no reproduzca audio. La telemetría agrega éxito privado de consulta sin contenido.

El banco reproduce la tanda física 3→4→5 partida a mitad del hoyo 4, la recompone, escribe los seis Gross en una transacción y comprueba la apertura visible para `cómo está el clima hoy`. Archivos exactos V351-R8: `index-grupal.html`, `api/voice-health.js`, `test-v351-r6-consecutive-holes-voice-score.mjs`, `test-v351-r1-hole1-voice-score-render.mjs`, `test-v336-microphone-transport.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece intacta hasta PASS físico iPhone.

## V351-R9 · alternativas Safari y AI Gateway primero · 28 de agosto de 2026

La prueba física de las 02:28–02:29 rechazó R8. El deployment `dpl_4iP4Up3kaqzBUobpToUiEeiS92E2` registró `ambiguous_score` a las 08:28:53 UTC, un score individual aplicado a las 08:29:06 y `/api/universal-ai` HTTP 503 a las 08:29:20 porque el proveedor directo devolvió `credit_balance_exhausted`. La respuesta visible confirmó que Internet funcionaba, pero el servicio externo no.

R9 solicita hasta cinco hipótesis de Safari y construye un máximo de 16 combinaciones. Si sólo una semántica válida existe, la entrega al mismo escritor oficial; si dos alternativas válidas cambian cualquier Gross, no escribe. AI Gateway pasa a ser la primera ruta, el proveedor directo queda como failover y el saldo agotado deja de repetirse. La verificación visual del primer Preview demostró que el OIDC del build no bastaba en runtime; `@vercel/oidc` recupera ahora el token desde el contexto de la Function y los follow-ups reutilizan ese mismo token. `test-v351-r9-live-universal-ai.mjs` exige HTTP 200 real en el build cuando hay credenciales.

Archivos exactos V351-R9: `index-grupal.html`, `api/universal-ai.js`, `package.json`, `test-v321-ai-universal-infinity.mjs`, `test-v351-r6-consecutive-holes-voice-score.mjs`, `test-v335-response-caliber.mjs`, `test-v351-r9-live-universal-ai.mjs`, `audit-project.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. El test V321 aísla expresamente el fallback directo de las credenciales automáticas del Gateway; la puerta viva R9 valida el Gateway por separado. Producción permanece intacta hasta puerta viva, Preview READY y PASS físico iPhone.

## V351-R10 · captura larga de Safari y tráfico abreviado · 28 de agosto de 2026

La prueba física de las 06:21–06:23 rechazó R9. Registro y scores individuales funcionaron, pero la tanda multi-hoyo terminó en `missing_score`; la consulta de tráfico llegó al servidor y respondió HTTP 502. El transporte alternativo seguía limitado a una sola emisión y su cierre manual descartaba la cola pendiente; además la ruta abreviada enviaba lugares incompletos al proveedor.

R10 conserva la escucha de Safari durante toda la tanda, finaliza tras tres segundos de silencio o al segundo toque y procesa una sola vez la cola acumulada. El parser y el escritor transaccional siguen rechazando cualquier tanda incompleta. La ruta de tráfico canonicaliza los alias locales inequívocos y el panel siempre muestra resultado, error seguro o timeout.

Archivos exactos V351-R10: `index-grupal.html`, `api/universal-ai.js`, `test-v324-real-traffic.mjs`, `test-v336-microphone-transport.mjs`, `test-v351-r6-consecutive-holes-voice-score.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece intacta hasta Preview READY, tráfico vivo y PASS físico iPhone.

## V351-R11 · recuperación del cierre natural de Safari · 28 de agosto de 2026

La prueba física rechazó R10: Registro y un hoyo individual continuaron funcionando, pero Safari volvió a cortar multihoyos y preguntas generales. La causa quedó en el transporte: iOS puede emitir `onend` después del primer bloque aunque `continuous=true`; R10 finalizaba en ese instante y las pruebas no simulaban esa secuencia.

R11 conserva el fragmento pendiente, reinicia el reconocimiento antes de que venza la ventana de silencio, acumula alternativas entre sesiones y selecciona el Score válido con mayor número de pares jugador+hoyo. Las preguntas generales usan la frase completa más larga. Una contradicción completa de Gross permanece bloqueada.

Archivos exactos V351-R11: `index-grupal.html`, `test-v351-r11-ios-natural-end-recovery.mjs`, `test-v351-r6-consecutive-holes-voice-score.mjs`, `audit-project.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece intacta hasta Preview READY y PASS físico iPhone.

## V351-R12 · hipótesis principal y recuperación del transporte iOS · 28 de agosto de 2026

La prueba física rechazó R11. Los logs del deployment `dpl_87UgZGjoKEnqs8CMENcA1gS1iWrt` muestran el score individual aplicado, la tanda posterior rechazada como `ambiguous_score` y otra apertura sin cierre observable; la pantalla terminó en “NO HAY UN MICRÓFONO DISPONIBLE”. El selector daba el mismo peso a alternativas secundarias y el cliente omitía del envío varios eventos de error/reinicio.

R12 reconoce como autoritativa la hipótesis principal completa de Safari. Las alternativas quedan como recuperación cuando la principal no es válida y una contradicción sin principal continúa bloqueada. `onend` y `audio-capture` liberan la instancia anterior, crean una nueva y conservan la orden; el error físico tiene dos reintentos máximos y telemetría cerrada sin contenido privado.

Archivos exactos V351-R12: `index-grupal.html`, `api/voice-health.js`, `test-v351-r12-ios-primary-transport-recovery.mjs`, `test-v351-r11-ios-natural-end-recovery.mjs`, `test-v351-r6-consecutive-holes-voice-score.mjs`, `audit-project.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece intacta hasta Preview READY y PASS físico iPhone.

## V351-R13 · tarjeta viva después de cerrar y reabrir · 28 de agosto de 2026

La prueba física de R12 confirmó Registro, dictado conjunto de dos hoyos y una respuesta visible de clima. R12 queda rechazada porque el redirect raíz agrega `?inicio=1` y el arranque abría Registro vacío incluso con una ronda configurada. La ronda podía seguir en almacenamiento, pero la pantalla montada violaba la regla operativa de mantener la tarjeta viva hasta confirmar una nueva ronda.

R13 sólo abre Registro automático cuando no existe una ronda activa. Al cerrar y volver a abrir, monta la misma modalidad, jugadores, hoyos y Gross; pulsar `NUEVA RONDA` conserva la tarjeta vigente y sólo `INICIAR RONDA` la reemplaza. Además, el micrófono de Registro permite preguntas generales explícitas y conserva local cualquier listado inválido, resolviendo la frontera que impedía que la consulta alcanzara `/api/universal-ai`. Se renueva la caché de la PWA y el nuevo banco reproduce Match Play con dos jugadores y varios hoyos a través de persistencia y reapertura. El inventario visual se compacta antes del sello para evitar truncamiento y se verifica renderizado en portada, página alta y cierre.

Archivos exactos V351-R13: `index-grupal.html`, `service-worker.js`, `test-v351-r13-live-round-reload.mjs`, `test-v311-neutral-match-home-link.mjs`, `test-v281-pwa-installation.mjs`, `test-v351-r12-ios-primary-transport-recovery.mjs`, `test-v351-r6-consecutive-holes-voice-score.mjs`, `audit-project.mjs`, `scripts/rebuild-inventory-pdfs.py`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece intacta hasta Preview READY y PASS físico de cierre/reapertura.
