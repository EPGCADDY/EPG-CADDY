...
## Corrección V321 · conversación sostenida y recuperación sin silencio

La prueba real de iPhone encontró dos fallas que los controles anteriores no detectaban: el propio cliente apagaba el micrófono tres segundos después de cada respuesta y una investigación web abortada devolvía HTTP 502. V321 elimina ese cierre breve; la escucha continúa de turno en turno y sólo se apaga después de 30 minutos completos sin actividad. Al tocar de nuevo, una conexión WebRTC sana se reutiliza en vez de destruirse. Si Inicio o Tarjeta no reciben la transcripción final, un watchdog limpia el bloqueo y devuelve `● ESCUCHANDO`.

`api/research.js` amplía el margen de investigación y transforma timeout, error del proveedor o respuesta vacía en un resultado recuperable HTTP 200. `index-grupal.html` obliga al Caddie a decir brevemente que no pudo verificar y a continuar escuchando, sin quedarse callado. Un error de generación tampoco apaga un transporte todavía sano. `test-v321-sustained-caddie.mjs` ejecuta 24 ciclos consecutivos, reapertura con sesión reutilizable, cierre reglamentario de 30 minutos y las tres salidas de investigación: éxito, proveedor no disponible y timeout. `test-v312-general-caddie.mjs` y `test-v320-universal-100-domains.mjs` conservan score protegido y 100 áreas universales.

Archivos de aplicación y auditoría: `api/research.js`, `audit-project.mjs`, `index-grupal.html`, `service-worker.js`, `test-v321-sustained-caddie.mjs`, `test-v312-general-caddie.mjs`, `test-v320-universal-100-domains.mjs`, `test-stableford-ui.mjs`, `test-v272-definitive-operational-release.mjs`, `test-v274-complete-courses-voice-operations.mjs`, `test-v275-stable-live-voice-turns.mjs`, `test-v276-manual-hole-navigation.mjs`, `test-v277-official-round-corrections.mjs`, `test-v278-card-image-pdf-export.mjs`, `test-v279-local-card-library.mjs`, `test-v280-local-history-insights.mjs`, `test-v281-pwa-installation.mjs`, `test-v284-native-package-generation.mjs`, `test-v290-brand-icons-cleanup.mjs`, `test-v304-homogeneous-registration-actions.mjs`, `test-v305-history-navigation-zero-error.mjs` y `test-v307-match-arrows-format.mjs`. El control documental queda actualizado en `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`, `ROADMAP_OVERALL.md` y `ROADMAP_A_DETALLE.md`.
