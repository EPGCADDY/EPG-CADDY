...
La salida hablada de Comunicación Universal, clima y tráfico deja de usar Cedar/Onyx porque la prueba física reportó acento Spanglish. `index-grupal.html` selecciona exclusivamente voces instaladas con locale exacto `es-MX`, prioriza nombres masculinos cuando iOS los identifica y usa velocidad 0.90. El cierre por silencio baja a 1 segundo; la escucha vacía se recupera en 6 segundos; el micrófono conversacional se cierra tras 3 segundos inactivo y una respuesta bloqueada se recupera a los 12 segundos. Si el iPhone no tiene una voz México instalada, muestra la ruta de instalación y no reproduce otra voz. `service-worker.js` cambia la caché; los controles V312, V321, V322, V323, V325, V326, V356, V357, V361, V362, V369 y V370 bloquean tiempos, identidad V371 y cualquier regreso de Cedar, Onyx o `/api/voice-speech` dentro de esta salida. Registro, Score, cálculos, persistencia y Producción no cambian.
