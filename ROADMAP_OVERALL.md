...
Las capturas físicas `IMG_2190.jpeg`/`IMG_2191.png` rechazaron el candidato anterior. La recuperación heredada podía aceptar una ronda configurada sin jugadores y el cliente acreditaba voz cuando `play()` comenzaba, no cuando el audio terminaba. V365 exige `playableStoredRound()` para toda restauración, vuelve a Registro ante residuos vacíos y espera `ended`/`onend` con watchdog, cancelación y evento privado `browser_fallback_speech_completed`. La caché instalable se renueva y `test-v365-iphone-startup-playback.mjs` simula ambas regresiones. Archivos: `index-grupal.html`, `api/voice-health.js`, `service-worker.js`, `Intocables/intocables-gate.mjs`, `test-v260-round-points-player-return.mjs`, `test-v267-one-operational-line.mjs`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v361-synchronized-voice.mjs`, `test-v363-intocables-behavior.mjs`, `test-v365-iphone-startup-playback.mjs`, `audit-project.mjs`, `package.json` y documentos de control. Producción permanece intacta; Preview y PASS físico iPhone son bloqueantes.
