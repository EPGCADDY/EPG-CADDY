...
## V358 · score natural y AI UNIVERSAL de un toque · 28 de agosto de 2026

La prueba física V357 confirmó que el permiso y el micrófono del iPhone funcionaban: Registro transcribió y aplicó jugadores. A las 17:12:44 UTC la ronda también produjo `browser_fallback_transcript_ready`, pero terminó en `parser_rejected`, `entryCount:0`; no existió ninguna llamada posterior a `/api/universal-ai`. La inspección localizó dos fronteras: el parser todavía exigía variantes canónicas de hoyo/score y el botón global `AI ∞` sólo abría el panel, sin activar la escucha.

V358 conserva la apertura local dentro del gesto y amplía de forma segura el score hablado: recupera `hoy`, `oy` u `ollo` sólo cuando anteceden un número de hoyo; acepta `hoyo número uno`, ordinales, `hice`, `saqué`, `tiro(s)` y `golpe(s)`; y usa al siguiente jugador pendiente cuando el nombre se omite. Dos interpretaciones válidas distintas continúan bloqueadas y ningún score ambiguo se escribe. `AI ∞` abre el panel, prepara Cedar y activa el micrófono dentro del mismo toque; la respuesta hablada conserva `voiceOnly`, sin pregunta ni respuesta visible.

Archivos exactos V358: `.github/workflows/roadmap-gate.yml`, `AGENTS.md`, `audit-project.mjs`, `index-grupal.html`, `package.json`, `service-worker.js`, `test-v352-live.mjs`, `test-v353-live-hub.mjs`, `test-v354-voice-fallback.mjs`, `test-v355-ios-audio-dictation.mjs`, `test-v356-voice-only-cedar-quality.mjs`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v358-ios-score-universal-physical-recovery.mjs`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md`, `scripts/rebuild-inventory-pdfs.py` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece intacta; banco, Preview, navegador y PASS físico iPhone son puertas separadas.
