...
# V407-R25 · compartir 24 h desde la Score Card · 09 de septiembre de 2026

- `index-grupal.html`: `COMPARTIR 24H` se muestra únicamente cuando el servidor confirma cuenta propietaria y `canShare=true`; cada toque crea un enlace nuevo, abre la hoja nativa de compartir —incluido WhatsApp— y no navega fuera de la Score Card.
- El mensaje entregado explica el alcance real del invitado: registrar de uno a seis jugadores y anotar scores por voz o escritura. El invitado nunca recibe el control de invitación y la API mantiene la autorización propietaria.
- `manual.html` y `test-v311-live-support-link.mjs`: SUPPORT conserva el manual interactivo en la misma pantalla con ampliación, navegación, lectura y velocidad; la corrección visual de sus gráficas sigue bajo el perfil editorial del Manual.
- `test-r18-owner-guest-24h-access.mjs`: candado positivo para creación/hoja nativa y negativo contra la navegación a la pantalla administrativa.
- Archivos modificados: `index-grupal.html`, `manual.html`, `test-r18-owner-guest-24h-access.mjs`, `test-v311-live-support-link.mjs`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md`, `CONTROL_PROYECTO_SCIRE/CONTINUIDAD_MAESTRA_LAB.md` y `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`. MAIN permanece intacta.
- Reedición Support: `docs/manual/v311/manual-pages-17-35.json` aclara registro opcional de uno a seis jugadores, alternativa manual ante voz no reconocida y corrección puntual de scores. `scripts/rebuild-manual-bets-live-data.py` evita PNG optimizados truncados; se regeneran `docs/manual/v311/page-19.png`, `docs/manual/v311/page-20.png`, `docs/manual/v311/page-21.png` y ambos PDF de 74 páginas. `test-v311-manual-semantic-coverage.mjs` bloquea la pérdida futura de esas explicaciones.
- `service-worker.js`: conserva únicamente la marca documental R18 exigida por la regresión de encabezado LIVE; no cambia release, caché, instalación ni funcionamiento de ACTUALIZAR.
- Corrección de ubicación: `index-grupal.html` incorpora `WHATSAPP · INVITAR 24H` dentro de `REGISTRO DE JUGADORES`, visible sólo tras autorización propietaria; conserva el acceso gemelo desde la Score Card. La guía dice `REGISTRA SÓLO A UNO O AL GRUPO · HASTA 6`. `test-r18-owner-guest-24h-access.mjs` bloquea ubicación, texto y manejador común.
- Publicación LAB: `docs/manual/v311/page-00.png` conserva 2160×4320 y 300 dpi con compresión indexada estable; evita truncamiento de transporte sin cambiar diseño, texto ni funcionamiento.
- Construcción reproducible: Vercel recompone las páginas 19–21, ambos PDF del Manual y los tres inventarios antes de ejecutar `audit-project.mjs`; así el artefacto publicado coincide con las fuentes aprobadas aunque Git transporte los PDF anteriores.
