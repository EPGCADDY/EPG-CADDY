...
## V358 · sincronización única, score progresivo y ronda persistente · 28 de agosto de 2026

V358 integra sin sustituciones las dos correcciones V357 que llegaron desde conversaciones concurrentes. Conserva la apertura del micrófono dentro del gesto de iPhone, cinco alternativas, reinicios y reintentos; además entrega cada score completo al escritor incremental durante `onresult`. Al reconocer un hoyo, la celda se guarda y se vuelve visible mientras el micrófono continúa escuchando los siguientes. Una selección final ambigua revierte la escritura provisional; una selección válida se sella sin duplicados.

Registro sigue aplicando jugadores localmente y enruta únicamente preguntas generales explícitas a AI UNIVERSAL. En Score, el parser local conserva prioridad y cualquier pregunta general pasa a AI UNIVERSAL `voiceOnly`, con Cedar 1.15 y sin transcripción visible. La ronda configurada sobrevive al cierre, reapertura y `?inicio=1`; sólo `INICIAR RONDA` crea otra tarjeta.

Archivos exactos V358: `.github/workflows/roadmap-gate.yml`, `AGENTS.md`, `api/voice-health.js`, `audit-project.mjs`, `index-grupal.html`, `package.json`, `service-worker.js`, `scripts/rebuild-inventory-pdfs.py`, `test-v311-neutral-match-home-link.mjs`, `test-v336-microphone-transport.mjs`, `test-v352-live.mjs`, `test-v353-live-hub.mjs`, `test-v354-voice-fallback.mjs`, `test-v355-ios-audio-dictation.mjs`, `test-v356-voice-only-cedar-quality.mjs`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v357-synchronized-progressive-voice.mjs`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`.

Los candados acumulados son `test-v357-ios-voice-transport-recovery.mjs` y `test-v357-synchronized-progressive-voice.mjs`, además de V354–V356, tráfico/clima, motor, modalidades, LIVE, Manual e inventario. Producción permanece exactamente en `0dc1ba7a62b6bd6aec92752c539ca641cf950e26`; Preview, navegador y prueba física iPhone siguen separados.
