...
## V363 · recuperación OIDC y diagnóstico privado de tráfico · 28 de agosto de 2026

La verificación viva de V362 confirmó tres hechos separados: OpenAI directo devuelve `credit_balance_exhausted`, el respaldo Gateway no se intentaba porque el token no estaba disponible como variable estática y Google Routes devolvía un error upstream sin conservar su clasificación. V363 usa la API oficial `@vercel/oidc` para recuperar el token desde el contexto de la función cuando no existe `AI_GATEWAY_API_KEY` ni `VERCEL_OIDC_TOKEN`; AI UNIVERSAL y Cedar/Onyx comparten ese único resolvedor. Una cadena vacía explícita continúa desactivando el respaldo en pruebas deterministas.

Google Routes conserva la respuesta pública genérica, pero registra únicamente HTTP, `providerStatus` y código numérico. No registra origen, destino, coordenadas, clave ni mensaje del proveedor. El banco V363 reproduce `PERMISSION_DENIED`, exige OIDC dinámico y prueba negativamente toda fuga privada. Este candidato no declara resueltos AI, voz ni tráfico hasta repetir consultas reales en Preview y luego voz/score en iPhone.

Archivos exactos V363: `.github/workflows/roadmap-gate.yml`, `api/_lib/traffic.js`, `api/_lib/vercel-gateway-auth.js`, `api/universal-ai.js`, `api/voice-speech.js`, `audit-project.mjs`, `index-grupal.html`, `package.json`, `service-worker.js`, `test-v352-live.mjs`, `test-v353-live-hub.mjs`, `test-v354-voice-fallback.mjs`, `test-v355-ios-audio-dictation.mjs`, `test-v356-voice-only-cedar-quality.mjs`, `test-v357-ios-voice-transport-recovery.mjs`, `test-v357-synchronized-progressive-voice.mjs`, `test-v358-active-round-reopen.mjs`, `test-v358-ios-score-universal-physical-recovery.mjs`, `test-v359-ios-score-parser-recovery.mjs`, `test-v361-synchronized-voice.mjs`, `test-v362-physical-voice-recovery.mjs`, `test-v363-external-service-recovery.mjs`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/REGISTRO_REINCIDENCIAS_CALIDAD.md`, `CONTROL_PROYECTO_SCIRE/01_DIRECTRICES_PEDIDOS_Y_ORDENES_PENDIENTES/COLA_DE_PENDIENTES.md`, `GOLF_SCORE_CARD_GT_PENDING_MATRIX.md`, `CONTROL_PROYECTO_SCIRE/MAPA_MAESTRO_DE_ARCHIVOS.md`, `ROADMAP_OVERALL.md`, `ROADMAP_A_DETALLE.md`, `scripts/rebuild-inventory-pdfs.py`, `Inventario_Golf_Score_Card_GT_OVERALL_V311.pdf`, `Inventario_Golf_Score_Card_GT_A_DETALLE_V311.pdf`, `Inventario_Golf_Score_Card_GT_POR_IMAGENES_Y_RUBROS_V311.pdf` y `CONTROL_PROYECTO_SCIRE/INVENTARIOS_V311.lock.json`. Producción permanece exactamente en `0dc1ba7a62b6bd6aec92752c539ca641cf950e26`.
...
### V365-R5 · contrato completo de autenticación Gateway

R4 confirmó que la versión del modelo no sustituye la versión del transporte. El cliente oficial `@ai-sdk/gateway` agrega también `ai-gateway-protocol-version: 0.0.1` y `ai-gateway-auth-method`; `api/voice-speech.js` reproduce ahora ambos, distinguiendo API key de OIDC sin registrar credenciales. `test-v365-multiprovider-male-voice.mjs` e `Intocables/intocables-gate.mjs` exigen el contrato completo antes de volver a probar MP3.
