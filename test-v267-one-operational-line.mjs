...
assert.match(html,/const ALLOWED_SPEECH_REASONS=new Set\(\["closure","query","query_accumulated"\]\)/);
...
const parseScoreSequenceTranscript=new Function("normalizeSpeech","QUERY_WORDS","CORRECTION_WORDS","SCORE_FILLERS","SCORE_WORDS","skipScoreFillers","parseSpanishNumberTokens","matchPlayerAt","readGrossAt","canonicalPlayerNameKey","playerByRef",`${parserSource};return parseScoreSequenceTranscript`)(normalizeSpeech,QUERY_WORDS,CORRECTION_WORDS,SCORE_FILLERS,SCORE_WORDS,skipScoreFillers,parseSpanishNumberTokens,matchPlayerAt,readGrossAt,canonicalPlayerNameKey,playerByRef);
...
const applyLiteralScores=new Function("recordScore","recordScores",`${applySource};return applyLiteralScores`)(()=>{single++;return{ok:true}},({entries})=>{batch++;return{ok:true,count:entries.length}});
