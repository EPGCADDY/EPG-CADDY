...
assert.ok(manifest.icons.some(icon=>icon.sizes==="1254x1254"&&/image\/jpeg/.test(icon.type)));
assert.match(html,/rel="manifest" href="\/manifest\.webmanifest"/);
assert.match(html,/apple-mobile-web-app-capable/);
assert.match(html,/V281-PWA-OFFICIAL-INSTALL/);
assert.match(html,/id="installAppButton"/);
assert.match(html,/serviceWorker\.register\("\/service-worker\.js"/);
assert.match(worker,/const CACHE_NAME="gscg-mobile-v281"/);
