...
|---|---|---|---|
| P0 | Anuncios automáticos de vueltas y total Stableford | V266 implementado en rama local y cubierto por prueba automatizada; publicación y prueba física pendientes | Control manual y control de voz deben terminar en el mismo cálculo, persistencia y cierre hablado. Al completar el hoyo 9 debe anunciar una sola vez `Primera vuelta` con Nombre, Gross y Puntos de cada jugador. Al completar el hoyo 18 debe anunciar una sola vez `Segunda vuelta` con Nombre, Gross y Puntos de cada jugador y después `Total` con los acumulados de los 18 hoyos. Si la sesión de voz no está disponible en el primer intento debe recuperarla y reintentar sin alterar la tarjeta. |
