(function(root,factory){
  ...
  function hasScore(round,hole){return (round?.players||[]).some(p=>Number.isFinite(timestamp(p?.holes?.[hole]?.updatedAt)))}
  function isNineHoleGap(round){return !!(round?.configured&&!round?.endedAt&&hasScore(round,9)&&!hasScore(round,10))}
  function nineHoleAnchor(round){return scoreTime(round,9)??0}
  function remainingMs(round,now=Date.now()){
    if(isNineHoleGap(round))return Math.max(0,NINE_HOLE_WAIT_MS-Math.max(0,Number(now)-nineHoleAnchor(round)));
  ...
  function stopEndAt(round,now=Date.now()){
    if(isNineHoleGap(round)&&shouldStop(round,now))return nineHoleAnchor(round);
    return null;
  }
