(function(root,factory){
  ...
  function fourBallGlobalCard(snapshot){
    const finalHole=Number(snapshot.fourBall?.decidedAt)||18,rows=snapshot.players.map((player,index)=>{const teamIndex=fourBallTeamIndex(index),teamClass=teamIndex===0?"team-green":"team-gold",divider=index===2?`<tr class="pair-divider" aria-hidden="true"><td colspan="21"></td></tr>`:"";return`${divider}<tr><td>${esc(player.name)}</td><td class="${teamClass}">${FOUR_BALL_TEAM_NAMES[teamIndex]}</td><td>${player.handicap}</td>${Array.from({length:18},(_,offset)=>{const hole=offset+1,result=fourBallHole(snapshot,hole),standing=fourBallStanding(snapshot,teamIndex,hole),score=player.holes?.[hole],best=result.teamBest?.[teamIndex]?.playerIndexes?.includes(index);return`<td class="match-${standing.state}">${hole<=finalHole&&score?`${score.gross}/${score.net}${best?'<br><span class="best-ball">★ MEJOR</span>':""}<br>${matchSymbol(standing.state)} ${standing.label}`:"—"}</td>`}).join("")}</tr>`}).join("");
    return shell("Tarjeta Global Four Ball",snapshot,`<div class="match-result four-ball-result">${esc(snapshot.fourBall?.resultLabel||snapshot.fourBall?.label||"FOUR BALL")}</div><div class="table-wrap"><table><thead><tr><th>JUGADOR</th><th>PAREJA</th><th>HCP</th>${Array.from({length:18},(_,i)=>`<th>${i+1}<br>G/N</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table></div><p>Una o dos parejas, con HCP individual y resultado separado por pareja. ★ identifica el mejor Neto de cada pareja. Cuando participan dos parejas, sus mejores Netos se comparan y el marcador acumulado se mantiene hasta que cambia el resultado.</p>`)
  }
  function fourBallPersonalCard(snapshot,player){
    const index=snapshot.players.findIndex(item=>item.id===player.id),teamIndex=fourBallTeamIndex(index),mateIndex=teamIndex===0?(index===0?1:0):(index===2?3:2),rivalIndexes=snapshot.players.length===4?(teamIndex===0?[2,3]:[0,1]):[],finalHole=Number(snapshot.fourBall?.decidedAt)||18,rows=Array.from({length:finalHole},(_,offset)=>{const hole=offset+1,result=fourBallHole(snapshot,hole),standing=fourBallStanding(snapshot,teamIndex,hole),score=player.holes?.[hole],mate=snapshot.players[mateIndex]?.holes?.[hole],rivalBest=result.teamBest?.[1-teamIndex]?.score,best=result.teamBest?.[teamIndex]?.playerIndexes?.includes(index);return`<tr><td>${hole}</td><td>${score?.gross??"—"}</td><td>${score?.net??"—"}${best?' <span class="best-ball">★</span>':""}</td><td>${mate?.net??"—"}</td><td>${rivalBest??"—"}</td><td class="match-${standing.state}">${matchSymbol(standing.state)} ${standing.label}</td></tr>`}).join("");
