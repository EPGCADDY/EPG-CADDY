(function(root,factory){
  ...
  function sideGamePanels(snapshot){
    ...
    const wolf=snapshot?.sideGames?.wolf,wolfResult=wolf?.result;if(wolf?.enabled===true&&wolfResult){const rows=(wolfResult.summaries||[]).map(item=>`<tr><td>${esc(item.name)}</td><td>${item.wolfHoles}</td><td>${item.wins}</td><td>${item.losses}</td><td>${item.ties}</td><td class="${item.balance>0?'under':item.balance<0?'over':''}">${money(item.balance)}</td></tr>`).join("");panels.push(`<section class="side-game-panel"><h2>WOLF · ${esc(String(wolf.scoreType||"net").toUpperCase())}</h2><p>Mejor bola del lado Wolf contra mejor bola del campo · pareja ×1 · solo base ×1 · Lone ×2 · Blind ×3 · cada ganador cobra a cada rival perdedor · empate ${wolf.tiePolicy==="carry"?"acumula como regla del grupo":"push"} · variante GSC GT de 3 a 6 jugadores.</p><div class="table-wrap"><table><thead><tr><th>JUGADOR</th><th>WOLF</th><th>GANÓ</th><th>PERDIÓ</th><th>EMPATÓ</th><th>SALDO</th></tr></thead><tbody>${rows}</tbody></table></div><p><b>LIQUIDACIÓN:</b> ${settlements(wolfResult)}</p></section>`)}
