(function(root,factory){
  ...
  const matchSymbol=status=>status==="won"?"↑":status==="lost"?"↓":"";
  function matchPlayGlobalCard(snapshot){
    ...
    return shell(`Tarjeta Global Match Play`,snapshot,`<div class="match-result">${esc(snapshot.matchPlay?.resultLabel||snapshot.matchPlay?.label||"MATCH PLAY")}</div><div class="table-wrap"><table><thead><tr><th>JUGADOR</th><th>HCP</th>${Array.from({length:18},(_,i)=>`<th>${i+1}<br>G/N</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table></div><p>↑ ganó · ↓ perdió · sin símbolo = empate. Cada hoyo se define por resultado Neto con handicap.</p>`);
  }
  ...
  function build(snapshot){
    ...
    const stableford=snapshot.mode==="stableford",matchPlay=snapshot.mode==="match_play";
    const global={kind:"global",mode:stableford?"stableford":matchPlay?"match_play":"stroke",name:stableford?`tarjeta-global-stableford-v${snapshot.version}.html`:matchPlay?`tarjeta-global-match-play-v${snapshot.version}.html`:`tarjeta-global-v${snapshot.version}.html`,html:stableford?stablefordGlobalCard(snapshot):matchPlay?matchPlayGlobalCard(snapshot):globalCard(snapshot)};
    const personal=snapshot.players.map(player=>({kind:"personal",mode:stableford?"stableford":matchPlay?"match_play":"stroke",playerId:player.id,name:stableford?`tarjeta-stableford-${slug(player.name)}-v${snapshot.version}.html`:matchPlay?`tarjeta-match-play-${slug(player.name)}-v${snapshot.version}.html`:`tarjeta-${slug(player.name)}-v${snapshot.version}.html`,html:stableford?stablefordPersonalCard(snapshot,player):matchPlay?matchPlayPersonalCard(snapshot,player):personalCard(snapshot,player),stats:stableford?stablefordPlayerStats(player):playerStats(player)}));
