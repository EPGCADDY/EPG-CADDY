(function(root,factory){
  ...
  function stablefordSummary(name,stats){
    ...
    return`${name} finalizó con ${stats.points} puntos Stableford y ${gross}. IN: ${stats.front.points} puntos. OUT: ${stats.back.points} puntos.`;
  }
  ...
  function stablefordPersonalCard(snapshot,player){
    ...
    const items=[["Gross",grossTotal],["Puntos",stats.points],["Gross IN",stats.front.grossCount===9?stats.front.gross:"—"],["Puntos IN",stats.front.points],["Gross OUT",stats.back.grossCount===9?stats.back.gross:"—"],["Puntos OUT",stats.back.points],["4 puntos",stats.pointCounts[4]],["3 puntos",stats.pointCounts[3]],["2 puntos",stats.pointCounts[2]],["1 punto",stats.pointCounts[1]],["0 puntos",stats.pointCounts[0]],["Fecha clasificatoria",snapshot.stablefordRoundNumber||"—"]].map(([key,value])=>`<div class="stat"><b>${key}</b><br>${value}</div>`).join("");
